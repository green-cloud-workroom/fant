...
작성: 2026-09-14. **2026-09-15 상태 정정: 서버 요약은 사용자 요청으로 중단했고 현재 프런트는 session 모드다. [중단 기록](SUMMARY_DISABLED_20260915.md), [배포 오류 복구](DEPLOYMENT_ASSET_RECOVERY_20260915.md)를 우선한다. 후속 속도 개선은 [기본 화면 즉시 전환 계획](INSTANT_PAGE_PLAN_20260915.md)을 따른다. 아래 서버 serve 계획·실행 설명은 당시 기록이며 현재 운영 상태가 아니다.**

이 문서는 다음 구현자가 위에서 아래로 실행할 정본이다. 체크박스는 실제 산출물과 증거가 있을 때만 완료한다. 아래 신설 표기는 계획 작성 당시 기준이다. 현재 구현과의 차이(queryEqual 식별, 서버 fingerprint 검증, 기존 다단계 쓰기 보존)는 전 메뉴 릴리스의 계획 대비 확정 사항을 우선한다. 작업 기록은 [실행 체크시트](PERFORMANCE_EXECUTION_CHECKLIST_20260914.md)를 사용한다.
...
후속 사용자 요청으로 구현·배포 및 월 추가 비용 1만원 이내 목표가 승인됐다. 다음 문장은 계획 작성 당시의 일반 승인 절차다. 단계00~10을 포함한 구현 착수는 후속 구현 지시 후 진행한다. 구현 승인이 있으면 프런트 개발·로컬 검증부터 진행할 수 있고, 운영 릴리스와 새 서버/스케줄러/규칙/비용 발생은 해당 실행 지시의 승인 범위를 확인한다. 승인이 이미 있으면 다시 묻지 않는다. 승인이 없다면 diff·테스트·정확한 배포 대상·예상 사용량을 준비한 뒤 최종 배포 단계에서만 확인한다. 서버 배포 승인이 미정이어도 승인된 구현의 로컬 작업은 진행한다.
