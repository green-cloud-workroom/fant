import{T as e,a as t,b as n,d as r,g as i,s as a,u as o,y as s}from"./index.esm-rHmxwfvm.js";import{n as c}from"./firebase-qGjqjNvO.js";import{n as l}from"./modalManager-Z4b2ihO9.js";import{G as u,L as d,h as f,k as p}from"./index-DRMzawyA.js";import"./activityLogs-DYC0UZD3.js";import{t as m}from"./closingGuard-Dfuw_srM.js";import{t as h}from"./sortable-Du2optuH.js";import{t as g}from"./pageResources-BE3e2wtt.js";import{t as _}from"./pageRefresh-D3ZSodaR.js";import{t as v}from"./pageCommand-BkcRKfco.js";import{t as y}from"./commandWrites-DtPED3QM.js";import{n as b,t as x}from"./recipe-LxfTZLbF.js";import{t as S}from"./pageStaff-Cns9055z.js";var C=[],w=null,T=`frozenProductCategoryCollapsed`,ee=[{key:`cat-product`,label:`고양이`,target:`cat`,kind:`product`},{key:`dog-product`,label:`강아지`,target:`dog`,kind:`product`},{key:`common-product`,label:`공용`,target:`common`,kind:`product`},{key:`cat-sample`,label:`고양이샘플`,target:`cat`,kind:`sample`},{key:`dog-sample`,label:`강아지샘플`,target:`dog`,kind:`sample`},{key:`common-sample`,label:`공용샘플`,target:`common`,kind:`sample`},{key:`sample-set`,label:`샘플세트`,target:null,kind:`sampleSet`}];async function E({force:e=!1}={}){let t=document.getElementById(`mainContent`);t.innerHTML=`<p style="padding:24px">동결제품 입고 로딩 중...</p>`;let n=await $.load(async e=>{let[t,n]=await Promise.all([D(e),S(e)]);return{products:t,staff:n}},{force:e,onChange:_($,E)});if(!(!n||!t.isConnected)&&(C=n.products,X=n.staff,k(),w)){let e=C.find(e=>e.id===w);e&&await U(e)}}async function D(e={getDocs:a,getDoc:t}){let n=r(s(c,`frozenProducts`),o(`sortOrder`));return(await e.getDocs(n)).docs.map(e=>({id:e.id,...e.data()}))}async function O(e){return(await a(r(s(c,`frozenLogs`),o(`timestamp`,`desc`)))).docs.map(e=>({id:e.id,...e.data()})).filter(t=>t.productId===e&&t.status!==`deleted`).slice(0,30)}function k(){let e=document.getElementById(`mainContent`);e.innerHTML=`
  ...
  `,B(),R(),document.getElementById(`btnNewProduct`)?.addEventListener(`click`,W)}function A(e){let t=e?.name||``;return[`cat`,`dog`,`common`].includes(e?.target)?e.target:t.startsWith(`고양이 `)?`cat`:t.startsWith(`강아지 `)?`dog`:`common`}function j(e){return[`product`,`sample`,`sampleSet`].includes(e?.kind)?e.kind:`product`}function M(e){let t=j(e);return t===`sampleSet`?`sample-set`:`${A(e)}-${t}`}function N(){try{let e=JSON.parse(localStorage.getItem(T)||`{}`);return e&&typeof e==`object`?e:{}}catch{return{}}}function P(e,t){let n=N();n[e]=t,localStorage.setItem(T,JSON.stringify(n))}function F(e){let t=u===`admin`||u===`office`;return`
...
차감된 봉투 재고가 복원됩니다.`,confirmText:`삭제`,danger:!0}))return;let l=t.dataset.logid,p=await i(n(c,`frozenLogs`,l));if(!p.exists()){alert(`입고 로그를 찾을 수 없습니다.`);return}let h=p.data();if(await m(h.date,r))return;if(h.ledgerId){let e=await i(n(c,`stockLedger`,h.ledgerId));if(e.exists()&&e.data().status===`active`){let t=e.data().items||[];for(let e of t){let t=await i(n(c,e.collection,e.docId));if(!t.exists())continue;let r=t.data()[e.field]||0;if(r!==e.after&&!await f({title:`재고 변동 감지`,message:`동결제품 입고 이후 ${e.label} 재고가 변경된 이력이 있습니다.\n입고 당시 차감분만 복원됩니다.\n\n강제 복원하시겠습니까?`,confirmText:`강제 복원`,danger:!0}))continue;let l=r-e.delta;await o(n(c,e.collection,e.docId),{[e.field]:l,updatedAt:new Date}),e.collection===`bagTypes`&&await a(s(c,`bagLogs`),{date:d(),timestamp:new Date,bagTypeId:e.docId,bagNameSnapshot:t.data().name||``,type:`autoDeductReverse`,qty:-e.delta,before:r,after:l,staffName:h.staffName||``,note:`동결제품 입고 삭제 복원 - ${h.productNameSnapshot||``}`})}await o(n(c,`stockLedger`,h.ledgerId),{status:`rolledBack`,rolledBackAt:new Date})}}else if(h.bagTypeId&&(h.deductedBagQty||0)>0){let e=await i(n(c,`bagTypes`,h.bagTypeId));if(e.exists()){let t=e.data().currentQty||0;await o(n(c,`bagTypes`,h.bagTypeId),{currentQty:t+h.deductedBagQty,updatedAt:new Date})}}await o(n(c,`frozenLogs`,l),{status:`deleted`});let g=h.componentDeductLogIds||[];for(let e of g)await o(n(c,`frozenLogs`,e),{status:`deleted`});r.isCurrent()&&(await U(e),alert(`삭제 완료!`))},{roles:[`admin`,`office`]}))})}function W(){Y(null)}function G(e){Y(e)}function K(e){return Array.isArray(e)?e.map(e=>({frozenProductId:e?.frozenProductId||``,qty:Number(e?.qty)||0})).filter(e=>e.frozenProductId&&e.qty>0):[]}function q(e){return C.filter(t=>j(t)===`sample`&&t.id!==e)}function te(e=``,t=null){return[`<option value="">샘플 선택</option>`,...q(t).map(t=>`<option value="${t.id}" ${e===t.id?`selected`:``}>${t.name}</option>`)].join(``)}function J(e={},t=null){let n=Number(e.qty)>0?Number(e.qty):1;return`
  ...
  `),document.getElementById(`btnSaveEditIncoming`).addEventListener(`click`,async()=>v($,async r=>{let{getDoc:i,addDoc:a,updateDoc:o}=y(r),l=document.getElementById(`m_expiry`).value,p=parseInt(document.getElementById(`m_qty`).value),h=document.getElementById(`m_staff`).value,g=document.getElementById(`m_note`).value;if(!p||p<=0){alert(`수량은 1개 이상이어야 합니다.`);return}if(await m(t.date,r))return;if(u!==`admin`&&u!==`office`){alert(`동결제품 입고 수정은 대표/사무실 계정만 가능합니다.`);return}t.qty;let _=t.deductedBagQty||0;if(t.ledgerId){let r=await i(n(c,`stockLedger`,t.ledgerId));if(r.exists()&&r.data().status===`active`){let l=r.data().items||[];for(let t of l){let r=await i(n(c,t.collection,t.docId));if(!r.exists())continue;let l=r.data()[t.field]||0;if(l!==t.after&&!await f({title:`재고 변동 감지`,message:`동결제품 입고 이후 ${t.label} 재고가 변경된 이력이 있습니다.\n입고 당시 차감분만 복원됩니다.\n\n강제 복원하시겠습니까?`,confirmText:`강제 복원`,danger:!0}))return;let u=l-t.delta;await o(n(c,t.collection,t.docId),{[t.field]:u,updatedAt:new Date}),t.collection===`bagTypes`&&await a(s(c,`bagLogs`),{date:d(),timestamp:new Date,bagTypeId:t.docId,bagNameSnapshot:r.data().name||``,type:`autoDeductReverse`,qty:-t.delta,before:l,after:u,staffName:h,note:`동결제품 입고 수정(롤백) - ${e.name}`})}await o(n(c,`stockLedger`,t.ledgerId),{status:`rolledBack`,rolledBackAt:new Date})}}else if(t.bagTypeId&&_>0){let e=await i(n(c,`bagTypes`,t.bagTypeId));if(e.exists()){let r=e.data().currentQty||0;await o(n(c,`bagTypes`,t.bagTypeId),{currentQty:r+_,updatedAt:new Date})}}let v=0,b=[];if(e.bagTypeId){let l=await i(n(c,`bagTypes`,e.bagTypeId));if(l.exists()){let i=l.data(),u=i.currentQty||0;if(u<p){if(alert(`봉투 재고가 부족합니다.\n현재 봉투 재고: ${u}장\n필요 수량: ${p}장\n\n수정이 중단되었습니다. 봉투 재고는 이전 상태로 이미 복원되었습니다.`),!r.isCurrent()||(closeModal(),!r.isCurrent()))return;await U(e);return}let d=u-p,f=new Date;await o(n(c,`bagTypes`,e.bagTypeId),{currentQty:d,updatedAt:f}),v=p;let m=await a(s(c,`bagLogs`),{date:t.date,timestamp:new Date,bagTypeId:e.bagTypeId,bagNameSnapshot:i.name,type:`autoDeduct`,qty:-p,before:u,after:d,staffName:h,note:`동결제품 입고 수정(재차감) - ${e.name}`});b.push({collection:`bagTypes`,docId:e.bagTypeId,field:`currentQty`,delta:-p,before:u,after:d,label:`${i.name} 봉투`,stockUpdatedAtSnapshot:f,bagLogId:m.id})}}let x=null;b.length>0&&(x=(await a(s(c,`stockLedger`),{actionType:`frozenProductIncoming`,actionId:t.id,timestamp:new Date,date:t.date,status:`active`,items:b})).id),await o(n(c,`frozenLogs`,t.id),{qty:p,expiryDate:l,staffName:h,note:g,deductedBagQty:v,ledgerId:x,updatedAt:new Date}),r.isCurrent()&&(closeModal(),r.isCurrent()&&(await U(e),alert(`수정 완료!`)))},{roles:[`admin`,`office`]}))}async function ae(){let e=await a(s(c,`frozenLogs`)),t={};return e.docs.forEach(e=>{let n=e.data();n.status!==`deleted`&&n.productId&&(t[n.productId]=(t[n.productId]||0)+Number(n.qty||0))}),t}async function oe(e){let t=d(),r=p(18),i=j(e)===`sampleSet`?K(e.components):[],a=i.length>0?await ae():{},o=e=>C.find(t=>t.id===e)?.name||`(삭제된 제품)`;Q(`
