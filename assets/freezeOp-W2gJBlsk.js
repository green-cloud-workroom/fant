import{C as e,_ as t,b as n,d as r,h as i,o as a,u as o,y as s}from"./index.esm-rHmxwfvm.js";import{n as c}from"./firebase-qGjqjNvO.js";import{H as l,P as u,g as d}from"./index-ihWgJTcM.js";var f=[`#4299E1`,`#48BB78`,`#ED8936`,`#9F7AEA`,`#F56565`,`#38B2AC`,`#ECC94B`,`#667EEA`,`#FC8181`,`#68D391`,`#63B3ED`,`#F6AD55`,`#B794F4`,`#FBD38D`,`#90CDF4`],p=45,m=3,h=Array(p).fill(null),g=null,_=null,v={},y=!1;function b(){return y?document.getElementById(`freezeOpContent`):document.getElementById(`mainContent`)}function x(){return l}function S(){let e=x();return e===`admin`||e===`office`}async function C(){y=!1;let e=document.getElementById(`mainContent`);e.innerHTML=`<div style="padding:24px;"><p>동결가동 로딩 중...</p></div>`;let[t,n]=await Promise.all([T(),E()]);document.getElementById(`mainContent`)===e&&k(t,n,{container:e,tabMode:!1})}async function w(){y=!0;let e=document.getElementById(`freezeOpContent`);if(!e)return;e.innerHTML=`<p style="color:#888;font-size:13px;padding:8px 0;">로딩 중...</p>`;let[t,n]=await Promise.all([T(),E()]);document.getElementById(`freezeOpContent`)===e&&k(t,n,{container:e,tabMode:!0})}async function T(){return(await a(r(s(c,`freezeOrders`),o(`date`,`desc`)))).docs.map(e=>({id:e.id,...e.data()}))}async function E(){return(await a(s(c,`frozenPanLots`))).docs.map(e=>({id:e.id,...e.data()})).filter(e=>!e.closed&&Number(e.remaining||0)>0)}function D(e){let t={};return e.forEach(e=>{t[e.productName]=(t[e.productName]||0)+Number(e.remaining||0)}),t}async function O(){let[e,t]=await Promise.all([T(),E()]);k(e,t,{container:b(),tabMode:y})}function k(e,t,n={}){let r=n.container||b();if(!r)return;let i=n.tabMode??y,a=S(),o=D(t),s=a?`<button class="btn-primary" id="btnCreateOrder">+ 발주서 제작</button>`:``,c=`
  ...
  `}function j(r,i){let a=Object.entries(r).sort((e,t)=>e[0].localeCompare(t[0])).map(([e,t])=>`<option value="${R(e)}">${R(e)} (${t}판)</option>`).join(``);z(`
  ...
  `);function o(){let e=0;document.querySelectorAll(`#fo_items .fo-qty`).forEach(t=>{e+=parseInt(t.value)||0});let t=document.getElementById(`fo_total`);t&&(t.textContent=e,t.style.color=e>45?`#e53e3e`:`#e67e22`)}function l(){let e=document.createElement(`div`);e.className=`fo-row`,e.style.cssText=`display:flex;gap:8px;margin-bottom:8px;align-items:center;`,e.innerHTML=`
    ...
    `,document.getElementById(`fo_items`).appendChild(e),e.querySelector(`.fo-name`).addEventListener(`change`,t=>{e.querySelector(`.fo-hint`).textContent=t.target.value?`재고 ${r[t.target.value]||0}판`:``}),e.querySelector(`.fo-qty`).addEventListener(`input`,o),e.querySelector(`.fo-del`).addEventListener(`click`,()=>{e.remove(),o()})}function f(){let e=document.createElement(`div`);e.className=`fo-row fo-virtual`,e.style.cssText=`display:flex;gap:8px;margin-bottom:8px;align-items:center;`,e.innerHTML=`
      ...
      <button class="fo-del btn-secondary" style="font-size:11px;padding:2px 8px;">✕</button>
    `,document.getElementById(`fo_items`).appendChild(e),e.querySelector(`.fo-qty`).addEventListener(`input`,o),e.querySelector(`.fo-del`).addEventListener(`click`,()=>{e.remove(),o()})}document.getElementById(`fo_addStock`).addEventListener(`click`,l),document.getElementById(`fo_addVirtual`).addEventListener(`click`,f),l(),document.getElementById(`fo_save`).addEventListener(`click`,async()=>{let a=document.getElementById(`fo_date`).value;if(!a){alert(`날짜를 입력해주세요.`);return}let o=[],l=!1;if(document.querySelectorAll(`#fo_items .fo-row`).forEach(e=>{let t=e.classList.contains(`fo-virtual`),n=(e.querySelector(`.fo-name`).value||``).trim(),i=parseInt(e.querySelector(`.fo-qty`).value)||0;if(!(!n||i<=0)){if(!t){let e=r[n]||0;i>e&&(alert(`${n}: 재고 ${e}판 / 발주 ${i}판 — 재고 부족`),l=!0)}o.push({productName:n,panCount:i,isVirtual:t})}}),l)return;if(o.length===0){alert(`발주 항목을 추가해주세요.`);return}let u=o.reduce((e,t)=>e+t.panCount,0);if(u>45&&!await d({title:`판수 초과`,message:`총 ${u}판은 45판 기준을 초과합니다. 그래도 저장하시겠습니까?`,confirmText:`저장`}))return;let f;try{f=B(o,i)}catch(e){alert(e.message);return}let p=t(c);f.forEach(t=>{let r=i.find(e=>e.id===t.lotId),a=r&&Number(r.remaining||0)-t.amount<=0;p.update(n(c,`frozenPanLots`,t.lotId),{remaining:e(-t.amount),...a?{closed:!0}:{},updatedAt:new Date})});let m=n(s(c,`freezeOrders`));p.set(m,{date:a,items:o,totalPans:u,deductions:f,status:`active`,layout:null,layoutConfirmedAt:null,layoutConfirmedBy:null,createdAt:new Date,createdBy:x()||``}),await p.commit(),closeModal(),await O(),alert(`발주서 저장 완료! 동결판 재고가 차감되었습니다.`)})}function M(e,t){let n=(e.items||[]).reduce((e,t)=>e+Number(t.panCount||0),0),r=(e.items||[]).map(e=>`
    <tr>
