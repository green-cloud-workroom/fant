import{a as e,b as t,d as n,s as r,u as i,y as a}from"./index.esm-rHmxwfvm.js";import{n as o}from"./firebase-qGjqjNvO.js";import{n as s}from"./modalManager-Z4b2ihO9.js";import{G as c,L as l,g as u,h as d,x as f}from"./index-DRMzawyA.js";import"./activityLogs-DYC0UZD3.js";import{t as p}from"./closingGuard-Dfuw_srM.js";import{t as m}from"./pageResources-BE3e2wtt.js";import{t as h}from"./pageRefresh-D3ZSodaR.js";import{t as g}from"./pageCommand-BkcRKfco.js";import{t as _}from"./commandWrites-DtPED3QM.js";import{disposeFreezeOpTab as v,renderFreezeOpInTab as y}from"./freezeOp-Cy-gjZ9O.js";import{t as b}from"./recipe-LxfTZLbF.js";import{i as x,n as S,t as C}from"./number-BQbRppqI.js";import{t as ee}from"./pageStaff-Cns9055z.js";var w=[],T=`breadPan`;async function E({force:e=!1}={}){T=`breadPan`,document.getElementById(`mainContent`).innerHTML=`<p style="padding:24px">동결판 재고 로딩 중...</p>`,await k({force:e})}async function te(t={getDocs:r,getDoc:e}){let s=n(a(o,`frozenPanStock`),i(`date`,`desc`));return(await t.getDocs(s)).docs.map(e=>({id:e.id,...e.data()}))}async function D(t={getDocs:r,getDoc:e}){return(await t.getDocs(a(o,`frozenPanLots`))).docs.map(e=>({id:e.id,...e.data()})).filter(e=>!e.closed&&Number(e.remaining||0)>0)}async function O(t={getDocs:r,getDoc:e}){return(await t.getDocs(a(o,`breadPanLots`))).docs.map(e=>({id:e.id,...e.data()})).filter(e=>!e.closed)}async function ne(t={getDocs:r,getDoc:e}){let s=n(a(o,`breadPanLogs`),i(`timestamp`,`desc`));return(await t.getDocs(s)).docs.map(e=>({id:e.id,...e.data()}))}async function re(t={getDocs:r,getDoc:e}){let s=n(a(o,`frozenPanLogs`),i(`timestamp`,`desc`));return(await t.getDocs(s)).docs.map(e=>({id:e.id,...e.data()}))}async function k({force:e=!1}={}){let t=document.getElementById(`mainContent`),n=await $.load(async e=>{let[t,n,r,i,a,o,s]=await Promise.all([te(e),D(e),O(e),ne(e),re(e),b(e),ee(e)]);return{rows:t,lots:n,breadPanLots:r,breadPanLogs:i,frozenPanLogs:a,recipes:o,staff:s}},{force:e,onChange:h($,k)});!n||!t?.isConnected||(w=n.recipes,X=n.staff,A(n.rows,n.lots,n.breadPanLots,n.breadPanLogs,n.frozenPanLogs))}function A(e,t,n,r,i){let a=document.getElementById(`mainContent`);e=e.filter(e=>e.type!==`work`);let o={};t.forEach(e=>{o[e.productName]||(o[e.productName]=[]),o[e.productName].push({date:e.date,remaining:e.remaining,source:e.source||null})});let s={};n.forEach(e=>{s[e.productName]||(s[e.productName]=[]),s[e.productName].push({date:e.date,remaining:e.remaining})});let c=x(n.reduce((e,t)=>e+(t.remaining||0),0)),l=S(c),u=t.reduce((e,t)=>e+(t.remaining||0),0);if(a.innerHTML=`
    <div class="page-wrap">
  ...
  `}function I(e,t){let n=document.getElementById(`btnBreadPanIncoming`);n&&n.addEventListener(`click`,()=>z());let r=document.getElementById(`btnBreadPanAdjust`);r&&r.addEventListener(`click`,()=>B());let i=document.getElementById(`btnAddWorkRow`);i&&i.addEventListener(`click`,()=>U(e,t))}function L(e,n){let r=document.getElementById(`btnTenderIn`);r&&r.addEventListener(`click`,()=>R());let i=document.getElementById(`btnFrozenPanAdjust`);i&&i.addEventListener(`click`,()=>V()),document.querySelectorAll(`.btn-order-confirm`).forEach(t=>{t.addEventListener(`click`,async()=>{let r=t.dataset.id,i=e.find(e=>e.id===r);if(!i||await p(i.date))return;let a=await Y({title:`발주 확인 — 담당자 선택`,message:`발주 확인을 진행할 담당자를 선택해주세요.`,groups:[`senior`,`office`]});a&&await ie(i,n,a)})}),document.querySelectorAll(`.btn-order-delete`).forEach(n=>{n.addEventListener(`click`,async()=>g($,async r=>{let{updateDoc:i}=_(r);if(!await d({title:`발주 행 삭제`,message:`발주 행을 삭제하시겠습니까?`,confirmText:`삭제`,danger:!0}))return;let a=e.find(e=>e.id===n.dataset.id);a&&await p(a.date),!r&&(await i(t(o,`frozenPanStock`,n.dataset.id),{status:`cancelled`}),r.isCurrent()&&await k())},{roles:[`admin`,`office`]}))}),document.querySelectorAll(`.btn-order-cancel`).forEach(t=>{t.addEventListener(`click`,async()=>{if(!await d({title:`발주 확인 취소`,message:`발주 확인을 취소하시겠습니까?
...
빵판 입고를 먼저 진행해주세요.`);return}Q(`
  ...
  `),document.getElementById(`btnSaveBreadPanAdjust`).addEventListener(`click`,async()=>g($,async e=>{let{getDoc:n,addDoc:r,updateDoc:i}=_(e),s=document.getElementById(`m_lot`).value,c=document.getElementById(`m_adjustType`).value,u=x(parseFloat(document.getElementById(`m_qty`).value)||0),d=document.getElementById(`m_reason`).value.trim(),f=document.getElementById(`m_staff`).value;if(!s){alert(`대상 lot을 선택해주세요.`);return}if(u<=0){alert(`수량은 0보다 커야 합니다.`);return}if(!d){alert(`사유를 입력해주세요.`);return}if(!f){alert(`담당자를 선택해주세요.`);return}let m=c===`plus`?u:-u,h=await n(t(o,`breadPanLots`,s));if(!h.exists()){alert(`lot을 찾을 수 없습니다.`);return}let g=h.data();if(g.date,await p(l(),e))return;let v=g.remaining||0,y=x(v+m);if(y<0){alert(`조정 후 잔량이 ${y}개가 됩니다.\n수동조정으로 음수 재고를 만들 수 없습니다.\n현재 ${v}개에서 최대 ${v}개까지만 감소 가능합니다.`);return}let b=new Date,S=l();await i(t(o,`breadPanLots`,s),{remaining:y,closed:y<=.005,updatedAt:b}),await r(a(o,`breadPanLogs`),{type:`adjust`,date:S,productName:g.productName,qty:m,before:v,after:y,lotId:s,lotDate:g.date,expectedFrozenQty:null,actualFrozenQty:null,diff:null,staffName:f,uid:null,note:null,reason:d,batchId:null,ledgerId:null,timestamp:b}),e.isCurrent()&&(closeModal(),e.isCurrent()&&(await k(),alert(`수동 조정 완료!`)))},{roles:[`admin`,`office`,`production`]}))}async function V(){let e=await D();if(e.length===0){alert(`조정할 동결판 lot이 없습니다.
