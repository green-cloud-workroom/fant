import{_ as e,b as t,d as n,h as r,i,n as a,o,u as s,y as c}from"./index.esm-rHmxwfvm.js";import{n as l}from"./firebase-qGjqjNvO.js";import{n as u}from"./modalManager-Z4b2ihO9.js";import{H as d,P as f,_ as p,g as m}from"./index-ihWgJTcM.js";import{n as h}from"./activityLogs-Jg2Ymepu.js";import{t as g}from"./closingGuard-BCSnBl5U.js";import{renderFreezeOpInTab as _}from"./freezeOp-W2gJBlsk.js";import{t as v}from"./recipe-Cc33zif5.js";import{i as y,n as b,t as x}from"./number-BaNMt7D8.js";var S=[],C=`breadPan`;async function w(){let e=document.getElementById(`mainContent`);e.innerHTML=`<div style="padding:24px;"><p>동결판 재고 로딩 중...</p></div>`;let[t]=await Promise.all([v(),Z()]);e.isConnected&&(S=t,C=`breadPan`,await O())}async function T(){return(await o(n(c(l,`frozenPanStock`),s(`date`,`desc`)))).docs.map(e=>({id:e.id,...e.data()}))}async function E(){return(await o(c(l,`frozenPanLots`))).docs.map(e=>({id:e.id,...e.data()})).filter(e=>!e.closed&&Number(e.remaining||0)>0)}async function D(){return(await o(c(l,`breadPanLots`))).docs.map(e=>({id:e.id,...e.data()})).filter(e=>!e.closed)}async function ee(){return(await o(n(c(l,`breadPanLogs`),s(`timestamp`,`desc`)))).docs.map(e=>({id:e.id,...e.data()}))}async function te(){return(await o(n(c(l,`frozenPanLogs`),s(`timestamp`,`desc`)))).docs.map(e=>({id:e.id,...e.data()}))}async function O(){let e=document.getElementById(`mainContent`),[t,n,r,i,a]=await Promise.all([T(),E(),D(),ee(),te()]);e?.isConnected&&k(t,n,r,i,a)}function k(e,t,n,r,i){let a=document.getElementById(`mainContent`);e=e.filter(e=>e.type!==`work`);let o={};t.forEach(e=>{o[e.productName]||(o[e.productName]=[]),o[e.productName].push({date:e.date,remaining:e.remaining,source:e.source||null})});let s={};n.forEach(e=>{s[e.productName]||(s[e.productName]=[]),s[e.productName].push({date:e.date,remaining:e.remaining})});let c=y(n.reduce((e,t)=>e+(t.remaining||0),0)),l=b(c),u=t.reduce((e,t)=>e+(t.remaining||0),0);a.innerHTML=`
    <div class="page-wrap">
  ...
  `}function F(e,t){let n=document.getElementById(`btnBreadPanIncoming`);n&&n.addEventListener(`click`,()=>R());let r=document.getElementById(`btnBreadPanAdjust`);r&&r.addEventListener(`click`,()=>z());let i=document.getElementById(`btnAddWorkRow`);i&&i.addEventListener(`click`,()=>H(e,t))}function I(e,n){let i=document.getElementById(`btnTenderIn`);i&&i.addEventListener(`click`,()=>L());let a=document.getElementById(`btnFrozenPanAdjust`);a&&a.addEventListener(`click`,()=>B()),document.querySelectorAll(`.btn-order-confirm`).forEach(t=>{t.addEventListener(`click`,async()=>{let r=t.dataset.id,i=e.find(e=>e.id===r);if(!i||await g(i.date))return;let a=await Y({title:`발주 확인 — 담당자 선택`,message:`발주 확인을 진행할 담당자를 선택해주세요.`,groups:[`senior`,`office`]});a&&await J(i,n,a)})}),document.querySelectorAll(`.btn-order-delete`).forEach(n=>{n.addEventListener(`click`,async()=>{if(!await m({title:`발주 행 삭제`,message:`발주 행을 삭제하시겠습니까?`,confirmText:`삭제`,danger:!0}))return;let i=e.find(e=>e.id===n.dataset.id);i&&await g(i.date)||(await r(t(l,`frozenPanStock`,n.dataset.id),{status:`cancelled`}),await O())})}),document.querySelectorAll(`.btn-order-cancel`).forEach(t=>{t.addEventListener(`click`,async()=>{if(!await m({title:`발주 확인 취소`,message:`발주 확인을 취소하시겠습니까?
차감된 동결판 재고가 복원됩니다.`,confirmText:`확인`,danger:!0}))return;let r=t.dataset.id,i=e.find(e=>e.id===r);if(!i||await g(i.date))return;let a=await Y({title:`발주 취소 — 담당자 선택`,message:`발주 취소를 진행할 담당자를 선택해주세요.`,groups:[`senior`,`office`]});a&&await ne(i,n,a)})})}function L(){let e=S.filter(e=>e.requiresSeparation!==!0);if(e.length===0){alert(`동결텐더 레시피가 없습니다.
...
레시피 관리 메뉴에서 동결건조 레시피의 "분리 작업 필요" 옵션을 끈 레시피를 추가해주세요.`);return}$(`
  ...
  `),document.getElementById(`btnSaveTenderIn`).addEventListener(`click`,async()=>{let e=document.getElementById(`m_recipe`).value.trim(),n=parseInt(document.getElementById(`m_qty`).value)||0,i=document.getElementById(`m_date`).value,o=document.getElementById(`m_staff`).value,s=document.getElementById(`m_note`).value.trim();if(!e){alert(`제품을 선택해주세요.`);return}if(n<=0){alert(`동결판 수는 0보다 커야 합니다.`);return}if(!i){alert(`날짜를 입력해주세요.`);return}if(!o){alert(`담당자를 선택해주세요.`);return}if(await g(i))return;let u=new Date;try{let d=await a(c(l,`frozenPanLots`),{productName:e,date:i,staffName:o,initialQty:n,remaining:n,closed:!1,source:`tenderIn`,sourceRefId:null,note:s||null,createdAt:u,updatedAt:u}),f=await a(c(l,`frozenPanLogs`),{type:`tenderIn`,date:i,productName:e,qty:n,before:0,after:n,lotId:d.id,staffName:o,uid:null,note:s||null,reason:null,batchId:null,ledgerId:null,timestamp:u});await r(t(l,`frozenPanLots`,d.id),{sourceRefId:f.id}),await a(c(l,`stockLedger`),{actionType:`frozenPanTenderIn`,actionId:f.id,timestamp:u,date:i,status:`active`,items:[{collection:`frozenPanLots`,docId:d.id,field:`remaining`,delta:n,before:0,after:n,label:`${e} 동결판 (텐더동결 입고)`,stockUpdatedAtSnapshot:u,isNewDoc:!0}]}),closeModal(),await O(),alert(`텐더동결 입고 완료!`)}catch(e){console.error(`showTenderInModal 저장 에러:`,e),alert(`저장 중 오류가 발생했습니다.\n\n${e.message}`)}})}function R(){let e=S.filter(e=>e.requiresSeparation===!0);if(e.length===0){alert(`동결생식 레시피가 없습니다.
...
레시피 관리 메뉴에서 동결건조 레시피의 "분리 작업 필요" 옵션을 켜주세요.`);return}$(`
  ...
  `),document.getElementById(`btnSaveBreadPanIncoming`).addEventListener(`click`,async()=>{let e=document.getElementById(`m_recipe`).value.trim(),t=y(parseFloat(document.getElementById(`m_qty`).value)||0),n=document.getElementById(`m_date`).value,r=document.getElementById(`m_staff`).value,i=document.getElementById(`m_note`).value.trim();if(!e){alert(`레시피를 선택해주세요.`);return}if(t<=0){alert(`빵판 수는 0보다 커야 합니다.`);return}if(!n){alert(`날짜를 입력해주세요.`);return}if(!r){alert(`담당자를 선택해주세요.`);return}if(await g(n))return;let o=new Date,s=await a(c(l,`breadPanLots`),{productName:e,date:n,staffName:r,initialQty:t,remaining:t,closed:!1,note:i||null,createdAt:o,updatedAt:o});await a(c(l,`breadPanLogs`),{type:`incoming`,date:n,productName:e,qty:t,before:0,after:t,lotId:s.id,lotDate:n,expectedFrozenQty:null,actualFrozenQty:null,diff:null,staffName:r,uid:null,note:i||null,reason:null,batchId:null,ledgerId:null,timestamp:o}),closeModal(),await O(),alert(`빵판 입고 완료!`)})}async function z(){let e=await D();if(e.length===0){alert(`조정할 빵판 lot이 없습니다.
빵판 입고를 먼저 진행해주세요.`);return}$(`
  ...
  `),document.getElementById(`btnSaveBreadPanAdjust`).addEventListener(`click`,async()=>{let e=document.getElementById(`m_lot`).value,n=document.getElementById(`m_adjustType`).value,o=y(parseFloat(document.getElementById(`m_qty`).value)||0),s=document.getElementById(`m_reason`).value.trim(),u=document.getElementById(`m_staff`).value;if(!e){alert(`대상 lot을 선택해주세요.`);return}if(o<=0){alert(`수량은 0보다 커야 합니다.`);return}if(!s){alert(`사유를 입력해주세요.`);return}if(!u){alert(`담당자를 선택해주세요.`);return}let d=n===`plus`?o:-o,p=await i(t(l,`breadPanLots`,e));if(!p.exists()){alert(`lot을 찾을 수 없습니다.`);return}let m=p.data();if(m.date,await g(f()))return;let h=m.remaining||0,_=y(h+d);if(_<0){alert(`조정 후 잔량이 ${_}개가 됩니다.\n수동조정으로 음수 재고를 만들 수 없습니다.\n현재 ${h}개에서 최대 ${h}개까지만 감소 가능합니다.`);return}let v=new Date,b=f();await r(t(l,`breadPanLots`,e),{remaining:_,closed:_<=.005,updatedAt:v}),await a(c(l,`breadPanLogs`),{type:`adjust`,date:b,productName:m.productName,qty:d,before:h,after:_,lotId:e,lotDate:m.date,expectedFrozenQty:null,actualFrozenQty:null,diff:null,staffName:u,uid:null,note:null,reason:s,batchId:null,ledgerId:null,timestamp:v}),closeModal(),await O(),alert(`수동 조정 완료!`)})}async function B(){let n=await E();if(n.length===0){alert(`조정할 동결판 lot이 없습니다.
동결판 입고를 먼저 진행해주세요.`);return}$(`
    ...
    <div class="form-group">
      ...
      <select id="m_lot">
        <option value="">선택</option>
        ${n.sort((e,t)=>{let n=e.productName.localeCompare(t.productName);return n===0?e.date.localeCompare(t.date):n}).map(e=>`<option value="${e.id}">${e.productName} / ${e.date} / 잔량 ${e.remaining}개</option>`).join(``)}
      </select>
    </div>
    <div class="form-row">
      <div class="form-group">
  ...
  `),document.getElementById(`btnSaveFrozenPanAdjust`).addEventListener(`click`,async()=>{let n=document.getElementById(`m_lot`).value,r=document.getElementById(`m_adjustType`).value,a=parseInt(document.getElementById(`m_qty`).value,10),o=document.getElementById(`m_reason`).value.trim(),s=document.getElementById(`m_staff`).value;if(!n){alert(`대상 lot을 선택해주세요.`);return}if(!Number.isInteger(a)||a<=0){alert(`수량은 0보다 큰 정수여야 합니다.`);return}if(!o){alert(`사유를 입력해주세요.`);return}if(!s){alert(`담당자를 선택해주세요.`);return}let u=r===`plus`?a:-a,d=await i(t(l,`frozenPanLots`,n));if(!d.exists()){alert(`lot을 찾을 수 없습니다.`);return}let p=d.data();if(await g(f()))return;let m=Number(p.remaining||0),h=m+u;if(h<0){alert(`조정 후 잔량이 ${h}개가 됩니다.\n수동조정으로 음수 재고를 만들 수 없습니다.\n현재 ${m}개에서 최대 ${m}개까지만 감소 가능합니다.`);return}let _=new Date,v=f(),y=e(l);y.update(t(l,`frozenPanLots`,n),{remaining:h,closed:h<=0,updatedAt:_}),y.set(t(c(l,`frozenPanLogs`)),{type:`adjust`,date:v,productName:p.productName,qty:u,before:m,after:h,lotId:n,staffName:s,uid:null,note:null,reason:o,batchId:null,ledgerId:null,timestamp:_}),await y.commit(),closeModal(),await O(),alert(`수동 조정 완료!`)})}function V(e){if(e.status===`cancelled`)return``;let t=d===`admin`||d===`office`,n=e.status===`confirmed`,r=(e.items||[]).reduce((e,t)=>e+(t.orderPanQty||0),0),i=r===45?`#2d7a3a`:r>45?`#e53e3e`:`#e67e22`,a=`
...
레시피 관리 메뉴에서 동결건조 레시피의 "분리 작업 필요" 옵션을 켜주세요.`);return}let i={};r.forEach(e=>{let t=n.filter(t=>t.productName===e.displayName&&t.remaining>.005).sort((e,t)=>e.date.localeCompare(t.date)),r=y(t.reduce((e,t)=>e+t.remaining,0)),a=t.length>0?t[0].remaining:0;i[e.displayName]={total:r,oldestRemaining:a}}),$(`
  ...
  `),G(i),K(),document.getElementById(`btnAddWorkItem`).addEventListener(`click`,()=>{document.getElementById(`workItems`).insertAdjacentHTML(`beforeend`,W(r,i)),G(i),K()}),document.getElementById(`btnSaveWorkRow`).addEventListener(`click`,async()=>{await U()})}async function U(){let e=document.getElementById(`m_date`).value,n=document.getElementById(`m_staff`).value,i=document.getElementById(`m_note`).value.trim(),o=Array.from(document.querySelectorAll(`.work-item`)).map(e=>({productName:e.querySelector(`.wi-name`).value.trim(),breadPanQty:y(parseFloat(e.querySelector(`.wi-bread`).value)||0),actualFrozenQty:parseInt(e.querySelector(`.wi-actual`).value)||0})).filter(e=>e.productName);if(!e){alert(`날짜를 입력해주세요.`);return}if(!n){alert(`담당자를 선택해주세요.`);return}if(o.length===0){alert(`제품을 1개 이상 입력해주세요.`);return}for(let e of o){if(e.breadPanQty<=0){alert(`${e.productName}: 출고 빵판 수가 입력되지 않았습니다.`);return}if(e.actualFrozenQty<=0){alert(`${e.productName}: 실제 산출 동결판 수가 입력되지 않았습니다.`);return}}if(await g(e))return;let s=await D(),u=[];for(let e of o){let t=y(s.filter(t=>t.productName===e.productName).reduce((e,t)=>e+t.remaining,0));e.breadPanQty>t&&u.push(`${e.productName}: 잔량 ${t}개 / 요청 ${e.breadPanQty}개`)}if(u.length>0){alert(`빵판 재고가 부족합니다.\n\n${u.join(`
`)}`);return}let d=`preprocess_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,f=new Date,p=[];try{for(let u of o){let o=x(u.breadPanQty),m=y(u.actualFrozenQty-o),h=u.breadPanQty,g=s.filter(e=>e.productName===u.productName&&e.remaining>.005).sort((e,t)=>e.date.localeCompare(t.date));for(let s of g){if(h<=.005)break;let _=y(Math.min(s.remaining,h)),v=s.remaining,b=y(v-_),x=new Date;await r(t(l,`breadPanLots`,s.id),{remaining:b,closed:b<=.005,updatedAt:x});let S=s.id===g[0].id;await a(c(l,`breadPanLogs`),{type:`preprocess`,date:e,productName:u.productName,qty:-_,before:v,after:b,lotId:s.id,lotDate:s.date,expectedFrozenQty:S?o:null,actualFrozenQty:S?u.actualFrozenQty:null,diff:S?m:null,staffName:n,uid:null,note:i||null,reason:null,batchId:d,ledgerId:null,timestamp:f}),p.push({collection:`breadPanLots`,docId:s.id,field:`remaining`,delta:-_,before:v,after:b,label:`${u.productName} 빵판 (${s.date})`,stockUpdatedAtSnapshot:x}),h=y(h-_)}let _=await a(c(l,`frozenPanLots`),{productName:u.productName,date:e,staffName:n,initialQty:u.actualFrozenQty,remaining:u.actualFrozenQty,closed:!1,source:`preprocess`,sourceRefId:d,note:i||null,createdAt:f,updatedAt:f});await a(c(l,`frozenPanLogs`),{type:`preprocess`,date:e,productName:u.productName,qty:u.actualFrozenQty,before:0,after:u.actualFrozenQty,lotId:_.id,staffName:n,uid:null,note:i||null,reason:null,batchId:d,ledgerId:null,timestamp:f}),p.push({collection:`frozenPanLots`,docId:_.id,field:`remaining`,delta:u.actualFrozenQty,before:0,after:u.actualFrozenQty,label:`${u.productName} 동결판 (신규 생성)`,stockUpdatedAtSnapshot:f,isNewDoc:!0})}await a(c(l,`stockLedger`),{actionType:`breadToFrozenPreprocess`,actionId:d,timestamp:f,date:e,status:`active`,items:p}),closeModal(),await O(),alert(`전처리 작업 저장 완료!`)}catch(e){console.error(`handleWorkRowSave 에러:`,e),alert(`저장 중 오류가 발생했습니다.\n\n${e.message}\n\n일부 데이터가 저장되었을 수 있으니 화면을 새로고침해서 확인해주세요.`)}}function W(e,t){return`
    <div class="work-item">
  ...
  `}function G(e){document.querySelectorAll(`.wi-del`).forEach(e=>{e.onclick=()=>{e.closest(`.work-item`).remove(),K()}}),document.querySelectorAll(`.work-item`).forEach(t=>{let n=t.querySelector(`.wi-name`),r=t.querySelector(`.wi-bread`);t.querySelector(`.wi-expected`);let i=t.querySelector(`.wi-actual`);t.querySelector(`.wi-diff`);let a=t.querySelector(`.wi-stock-info`);n.onchange=()=>{let i=n.value,o=e[i];!o||o.total<=0?(a.textContent=i?`⚠️ 빵판 lot 없음`:`제품을 선택하세요`,a.style.color=i?`#e53e3e`:`#888`,r.value=``,r.max=``):(a.textContent=`현재 잔량 ${o.total}개 (가장 오래된 lot ${o.oldestRemaining}개)`,a.style.color=`#2d7a3a`,r.value=o.oldestRemaining,r.max=o.total,q(t)),K()},r.oninput=()=>{q(t)},i.oninput=()=>{q(t)}})}function K(){let e=new Set;document.querySelectorAll(`.work-item .wi-name`).forEach(t=>{t.value&&e.add(t.value)}),document.querySelectorAll(`.work-item .wi-name`).forEach(t=>{let n=t.value;Array.from(t.options).forEach(t=>{t.value&&(t.disabled=t.value!==n&&e.has(t.value))})})}function q(e){let t=e.querySelector(`.wi-bread`),n=e.querySelector(`.wi-expected`),r=e.querySelector(`.wi-actual`),i=e.querySelector(`.wi-diff`),a=x(y(parseFloat(t.value)||0));n.value=a;let o=parseInt(r.value);if(isNaN(o)||r.value===``){i.textContent=`-`,i.style.color=`#888`;return}let s=y(o-a);i.textContent=`${s>0?`+`:``}${s}`,s===0?i.style.color=`#2d7a3a`:Math.abs(s)<=.5?i.style.color=`#888`:i.style.color=`#e67e22`}async function J(e,n,i){let o=e.items||[],s=[];for(let e of o){let t=n.filter(t=>t.productName===e.productName).reduce((e,t)=>e+t.remaining,0);e.orderPanQty>t&&s.push(`${e.productName}: 현재 재고 ${t}판 / 발주 ${e.orderPanQty}판`)}if(s.length>0){alert(`재고가 부족하여 발주 확인할 수 없습니다.\n\n${s.join(`
`)}\n\n발주를 삭제하거나 작업 행을 먼저 추가하세요.`);return}let u=[];for(let e of o){let i=e.orderPanQty,a=n.filter(t=>t.productName===e.productName).sort((e,t)=>e.date.localeCompare(t.date));for(let n of a){if(i<=0)break;let a=Math.min(n.remaining,i),o=n.remaining,s=o-a,c=new Date;await r(t(l,`frozenPanLots`,n.id),{remaining:s,closed:s<=0,updatedAt:c}),u.push({collection:`frozenPanLots`,docId:n.id,field:`remaining`,delta:-a,before:o,after:s,label:`${e.productName} (${n.date||`-`})`,stockUpdatedAtSnapshot:c}),i-=a}}let d=null;u.length>0&&(d=(await a(c(l,`stockLedger`),{actionType:`frozenPanOrder`,actionId:e.id,timestamp:new Date,date:e.date||``,status:`active`,items:u})).id),await r(t(l,`frozenPanStock`,e.id),{status:`confirmed`,ledgerId:d,confirmStaff:i,confirmedAt:new Date,updatedAt:new Date});let p=o.map(e=>`${e.productName} ${e.orderPanQty}판`).join(`, `);await h({action:`frozenPan`,subAction:`orderConfirm`,date:e.date||f(),staff:i,message:`동결판 발주 확인 — ${p} / 담당: ${i}`,details:{frozenPanStockId:e.id,orderDate:e.date||null,items:o,ledgerId:d}}),await O(),alert(`발주 확인 완료!`)}async function ne(e,n,a){if(d!==`admin`&&d!==`office`){alert(`발주 취소는 대표/사무실 계정만 가능합니다.`);return}let o=await p({title:`발주 취소`,message:`취소 후에는 차감된 동결판 재고가 복원됩니다.`,label:`취소 사유`,placeholder:`예: 재고 부족, 일정 변경`,required:!0,multiline:!0});if(o===null)return;if(e.ledgerId){let n=await i(t(l,`stockLedger`,e.ledgerId));if(n.exists()&&n.data().status===`active`){let a=n.data().items||[];for(let e of a){let n=await i(t(l,e.collection,e.docId));if(!n.exists())continue;let a=n.data()[e.field]||0;if(a!==e.after&&!await m({title:`로그아웃 확인`,message:`오늘 아직 마감되지 않았습니다.
로그아웃해도 자동으로 마감되지 않습니다.

로그아웃 하시겠습니까?`,confirmText:`로그아웃`}))return;let o=a-e.delta;await r(t(l,e.collection,e.docId),{[e.field]:o,closed:o<=0,updatedAt:new Date})}await r(t(l,`stockLedger`,e.ledgerId),{status:`rolledBack`,rolledBackAt:new Date})}}else{let i=e.items||[];for(let e of i){let i=n.filter(t=>t.productName===e.productName&&t.sourceRowId).sort((e,t)=>t.date.localeCompare(e.date)),a=e.orderPanQty;for(let n of i){if(a<=0)break;let i=Math.min(e.orderPanQty,a);await r(t(l,`frozenPanLots`,n.id),{remaining:n.remaining+i,closed:!1,updatedAt:new Date}),a-=i}}}await r(t(l,`frozenPanStock`,e.id),{status:`pending`,cancelReason:o,cancelStaff:a,cancelledAt:new Date,updatedAt:new Date});let s=e.items||[],c=s.map(e=>`${e.productName} ${e.orderPanQty}판`).join(`, `);await h({action:`frozenPan`,subAction:`orderCancel`,date:e.date||f(),staff:a,message:`동결판 발주 취소 — ${c} / 사유: ${o} / 담당: ${a}`,details:{frozenPanStockId:e.id,orderDate:e.date||null,items:s,reason:o}}),await O(),alert(`발주 취소 완료!`)}async function Y({title:e,message:t,groups:n}){return await Z(),new Promise(r=>{let i=document.getElementById(`modalOverlay`);i&&i.remove();let a=document.createElement(`div`);a.id=`modalOverlay`,a.className=`modal-overlay`,a.innerHTML=`
    ...
    `,document.body.appendChild(a);let o=e=>{a.remove(),r(e)};document.getElementById(`sp_cancel`).addEventListener(`click`,()=>o(null)),document.getElementById(`sp_ok`).addEventListener(`click`,()=>{let e=document.getElementById(`sp_staff`).value;if(!e){alert(`담당자를 선택해주세요.`);return}o(e)})})}var X={};async function Z(){Object.keys(X).length>0||await Promise.all([`senior`,`lead`,`office`].map(async e=>{let n=await i(t(l,`staffGroups`,e));n.exists()&&(X[e]=n.data().members||[])}))}function Q(e){let t=``;for(let n of e)(X[n]||[]).forEach(e=>{t+=`<option value="${e.name}">${e.name}</option>`});return t}function $(e){let t=document.getElementById(`modalOverlay`);t&&t.remove();let n=document.createElement(`div`);n.id=`modalOverlay`,n.className=`modal-overlay`,n.innerHTML=`<div class="modal-box">${e}</div>`,document.body.appendChild(n)}u(`frozenPan`,function(){let e=document.getElementById(`modalOverlay`);e&&e.remove()});export{w as renderFrozenPan};