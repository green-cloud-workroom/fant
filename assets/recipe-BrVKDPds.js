import{T as e,_ as t,b as n,d as r,g as i,i as a,o,u as s,y as c}from"./index.esm-rHmxwfvm.js";import{n as l}from"./firebase-qGjqjNvO.js";import{H as u,P as d,V as f,g as p}from"./index-ihWgJTcM.js";import{n as m}from"./activityLogs-Jg2Ymepu.js";import{t as h}from"./sortable-DoCboJo0.js";import{n as g,r as _,t as v}from"./supplement-ImEXygnG.js";var y=[],b=null,x=[],S=[],C=[],w=[],T=[{methodKey:`rotary`,label:`로터리`},{methodKey:`manual`,label:`수동`}];async function ee(){return(await o(r(c(l,`meatTypes`),s(`sortOrder`)))).docs.map(e=>({id:e.id,...e.data()}))}async function te(){return(await o(r(c(l,`bagTypes`),s(`sortOrder`)))).docs.map(e=>({id:e.id,...e.data()}))}async function ne(){let e=document.getElementById(`mainContent`);e.innerHTML=`<div style="padding:24px;"><p>레시피 로딩 중...</p></div>`;let t=await Promise.all([E(),ee(),te()]);e.isConnected&&([y,x,S]=t,D())}async function E(){return(await o(r(c(l,`recipes`),s(`sortOrder`)))).docs.map(e=>({id:e.id,...e.data()}))}function D(){let e=document.getElementById(`mainContent`);e.innerHTML=`
...
`)}\n\n레시피와 영양제 SKU ${n}개, 영양제 이력 ${r}건이 모두 삭제됩니다.\n진행하시겠습니까?`:r>0?`영양제 이력 ${r}건이 함께 삭제됩니다.\n레시피와 영양제 SKU ${n}개를 삭제하시겠습니까?`:n>0?`레시피와 영양제 SKU ${n}개를 삭제하시겠습니까?`:`${k(e)} 레시피를 삭제하시겠습니까?`}async function oe(e){if(e?.id){if(u!==`admin`&&u!==`office`){alert(`레시피 삭제는 대표/사무실 계정만 가능합니다.`);return}try{let r=await V(e);if(r.productionCount>0){alert(`이 레시피로 만든 생산 기록이 ${r.productionCount}건 있어 삭제할 수 없습니다.\n비활성화로 처리해주세요.`);return}if(r.scheduleCount>0){alert(`입고 예정 ${r.scheduleCount}건이 있어 삭제할 수 없습니다.`);return}if(!await p({title:`레시피 삭제`,message:ae(e,r.supplementSummaries),confirmText:`삭제`,danger:!0}))return;let i=t(l);i.delete(n(l,`recipes`,e.id)),r.supplementSummaries.forEach(e=>{i.delete(n(l,`supplementTypes`,e.sku.id)),e.stockExists&&i.delete(n(l,`supplementStock`,e.sku.id)),e.logDocs.forEach(e=>{i.delete(n(l,`supplementLogs`,e.id))})}),await i.commit();let a=r.supplementSummaries.length,o=r.supplementSummaries.reduce((e,t)=>e+t.stockQty,0),s=r.supplementSummaries.reduce((e,t)=>e+t.logDocs.length,0);await m({action:`recipe`,subAction:`delete`,date:d(),staff:A(),message:`레시피 삭제 — ${k(e)}`,details:{recipeId:e.id,recipeName:k(e),category:e.category===`freezeDry`?`dried`:`raw`,supplementSkuCount:a,supplementTotalQty:o,supplementLogCount:s}}),b=null,y=await E(),D(),alert(`레시피가 삭제되었습니다.`)}catch(e){console.error(`[recipe] delete failed:`,e),alert(`레시피 삭제 중 오류가 발생했습니다.`)}}}function Y(){return C.length?C.map((e,t)=>`
...
`),r=document.getElementById(`ingredientBody`),i=e.target.closest(`tr`),a=i?[...r.querySelectorAll(`tr`)].indexOf(i):0;n.forEach((e,t)=>{let n=e.split(`	`),i=n[0]?.trim()||``,o=n[1]?.trim()||``,s=x.find(e=>e.active!==!1&&e.name===i),c=!!s,l=s?.id||null,u=r.querySelectorAll(`tr`),d=a+t;if(u[d]){let e=u[d];e.querySelector(`.ing-name`).value=i,e.querySelector(`.ing-weight`).value=o;let t=e.querySelector(`.ing-weight-unit`);t&&(t.value=`g`,t.dataset.previousUnit=`g`);let n=e.querySelector(`.ing-linked`),r=e.querySelector(`.ing-meat-type`);n.checked=c,c?(r.style.display=``,r.value=l):(r.style.display=`none`,r.value=``)}else{let e=r.querySelectorAll(`tr`).length;r.insertAdjacentHTML(`beforeend`,Q({name:i,baseWeightG:o,weightDisplayUnit:`g`,linkedToInventory:c,meatTypeId:l},e)),$()}})}function de(){let e=document.querySelectorAll(`#ingredientBody tr`);return Array.from(e).map((e,t)=>{let n=e.querySelector(`.ing-linked`).checked,r=n&&e.querySelector(`.ing-meat-type`).value||null,i=e.querySelector(`.ing-weight-unit`)?.value===`kg`?`kg`:`g`,a=(parseFloat(e.querySelector(`.ing-weight`).value)||0)*(i===`kg`?1e3:1);return{id:Date.now().toString()+t,name:e.querySelector(`.ing-name`).value.trim(),baseWeightG:a,weightDisplayUnit:i,isProductionUnit:e.querySelector(`.ing-unit-radio`).checked,unitName:e.querySelector(`.ing-unit-name`).value.trim(),autoDeductInventory:n,linkedToInventory:n,meatTypeId:r,sortOrder:t}}).filter(e=>e.name)}async function fe(e){if(u!==`admin`&&u!==`office`){alert(`레시피 저장은 대표/사무실 계정만 가능합니다.`);return}let r=document.getElementById(`recipeName`).value.trim(),i=document.getElementById(`recipeCategory`).value,o=document.getElementById(`recipeTarget`).value,s=e?y.find(t=>t.id===e):null,p=Array.isArray(s?.unitPresets)?s.unitPresets:[],h=M(s?.productionMethods);if(!r||!i||!o){alert(`레시피명, 카테고리, 대상은 필수입니다.`);return}let g=document.getElementById(`recipeUsesSupplement`)?.checked??!0,_={name:r,displayName:(o===`cat`?`고양이 `:o===`dog`?`강아지 `:``)+r,category:i,target:o,color:document.getElementById(`recipeColor`).value,note:document.getElementById(`recipeNote`).value.trim(),active:e?s?.active!==!1:!0,sortOrder:e?s?.sortOrder??y.length:y.length,ingredients:de(),unitPresets:[...C],usesSupplement:g,version:1,updatedAt:new Date},x=g?_.unitPresets:[];if(i===`raw`){_.packWeightG=parseFloat(document.getElementById(`packWeightG`).value)||null,_.packsPerPlate=parseInt(document.getElementById(`recipePacksPerPlate`)?.value,10)||null;let e=document.getElementById(`recipeBagType`)?.value||``;if(!e){alert(`생식 레시피는 사용 봉투를 선택해야 합니다.`);return}_.bagTypeId=e;let t=L();if(t===null)return;_.productionMethods=t}else _.productionMethods=[];if(i===`freezeDry`){let e=document.getElementById(`requiresSeparation`).value===`true`;_.freezeDryBagCountPerUnit=parseFloat(document.getElementById(`freezeDryBagCount`).value)||null,_.breadPanCountPerUnit=e?parseFloat(document.getElementById(`breadPanCount`).value)||null:0,_.freezePanCountPerUnit=parseFloat(document.getElementById(`freezePanCount`).value)||null,_.requiresSeparation=e}let S=i===`raw`?R(h,_.productionMethods):[];try{if(e){let r=p.filter(e=>!x.includes(e)),i=await U({id:e,...s},r);if(i===!1){C=[...p],X();return}let o=await Promise.all(x.map(async t=>{let r=v(e,t),[i,o]=await Promise.all([a(n(l,`supplementTypes`,r)),a(n(l,`supplementStock`,r))]);return{unit:t,supplementTypeId:r,hasType:i.exists(),hasStock:o.exists()}})),c=new Map(o.map(e=>[e.unit,e])),u=t(l);u.update(n(l,`recipes`,e),_),z(u,e,S);let d=new Map((Array.isArray(i)?i:[]).map(e=>[e.unit,e]));x.forEach((t,r)=>{let i=v(e,t),a=B(e,_,t,r),o=c.get(t);p.includes(t)&&o?.hasType?u.update(n(l,`supplementTypes`,i),a):u.set(n(l,`supplementTypes`,i),{id:i,...a,createdAt:new Date,createdBy:f?.uid||null}),(!p.includes(t)||!o?.hasStock)&&u.set(n(l,`supplementStock`,i),{id:i,supplementTypeId:i,currentQty:0,updatedAt:new Date})}),r.forEach(t=>{let r=d.get(t),i=v(e,t);u.delete(n(l,`supplementTypes`,i)),u.delete(n(l,`supplementStock`,i)),(r?.logDocs||[]).forEach(e=>{u.delete(n(l,`supplementLogs`,e.id))})}),await u.commit(),b=e}else{_.createdAt=new Date,_.createdBy=f?.uid||null,_.updatedBy=f?.uid||null;let e=n(c(l,`recipes`)),r=t(l);r.set(e,_),z(r,e.id,S),x.forEach((t,i)=>{let a=v(e.id,t),o=B(e.id,_,t,i);r.set(n(l,`supplementTypes`,a),{id:a,...o,createdAt:new Date,createdBy:f?.uid||null}),r.set(n(l,`supplementStock`,a),{id:a,supplementTypeId:a,currentQty:0,updatedAt:new Date})}),await r.commit(),b=e.id}}catch(e){console.error(`[recipe] save failed:`,e),alert(`레시피 저장 중 오류가 발생했습니다.`);return}if(S.length>0)try{await m({action:`conversion`,subAction:`manualEdit`,date:d(),staff:A(),message:`환산값 변경 - ${_.displayName}`,details:{recipeId:b,recipeName:_.displayName,changes:S}})}catch(e){console.error(`[recipe] conversion activity log failed:`,e)}if(y=await E(),D(),b){let e=y.find(e=>e.id===b);e&&(J(e),document.querySelector(`[data-id="${b}"]`)?.classList.add(`active`))}alert(`저장되었습니다.`)}export{ne as renderRecipe};