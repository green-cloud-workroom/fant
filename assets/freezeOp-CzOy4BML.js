import{a as e,c as t,f as n,g as r,h as i,m as a,s as o,v as s}from"./index.esm-Cf9DaHbi.js";import{n as c}from"./firebase-BQPCO7kq.js";import{j as l,m as u,w as d}from"./index-CMgkQ4he.js";var f=[`#4299E1`,`#48BB78`,`#ED8936`,`#9F7AEA`,`#F56565`,`#38B2AC`,`#ECC94B`,`#667EEA`,`#FC8181`,`#68D391`,`#63B3ED`,`#F6AD55`,`#B794F4`,`#FBD38D`,`#90CDF4`],p=45,m=3,h=Array(p).fill(null),g=null,_=null,v={},y=!1;function b(){return y?document.getElementById(`freezeOpContent`):document.getElementById(`mainContent`)}function x(){return l}function S(){let e=x();return e===`admin`||e===`office`}async function C(){y=!1;let e=document.getElementById(`mainContent`);e.innerHTML=`<div style="padding:24px;"><p>동결가동 로딩 중...</p></div>`;let[t,n]=await Promise.all([T(),E()]);document.getElementById(`mainContent`)===e&&k(t,n,{container:e,tabMode:!1})}async function w(){y=!0;let e=document.getElementById(`freezeOpContent`);if(!e)return;e.innerHTML=`<p style="color:#888;font-size:13px;padding:8px 0;">로딩 중...</p>`;let[t,n]=await Promise.all([T(),E()]);document.getElementById(`freezeOpContent`)===e&&k(t,n,{container:e,tabMode:!0})}async function T(){return(await e(t(i(c,`freezeOrders`),o(`date`,`desc`)))).docs.map(e=>({id:e.id,...e.data()}))}async function E(){return(await e(i(c,`frozenPanLots`))).docs.map(e=>({id:e.id,...e.data()})).filter(e=>!e.closed&&Number(e.remaining||0)>0)}function D(e){let t={};return e.forEach(e=>{t[e.productName]=(t[e.productName]||0)+Number(e.remaining||0)}),t}async function O(){let[e,t]=await Promise.all([T(),E()]);k(e,t,{container:b(),tabMode:y})}function k(e,t,n={}){let r=n.container||b();if(!r)return;let i=n.tabMode??y,a=S(),o=D(t),s=a?`<button class="btn-primary" id="btnCreateOrder">+ 발주서 제작</button>`:``,c=`
  ...
  `}function j(e,t){let n=Object.entries(e).sort((e,t)=>e[0].localeCompare(t[0])).map(([e,t])=>`<option value="${R(e)}">${R(e)} (${t}판)</option>`).join(``);z(`
    <h3 class="modal-title">발주서 제작</h3>
    <div class="form-group">
      <label>동결건조 날짜 *</label>
      <input type="date" id="fo_date" value="${d()}" />
    </div>
  ...
  `);function o(){let e=0;document.querySelectorAll(`#fo_items .fo-qty`).forEach(t=>{e+=parseInt(t.value)||0});let t=document.getElementById(`fo_total`);t&&(t.textContent=e,t.style.color=e>45?`#e53e3e`:`#e67e22`)}function l(){let t=document.createElement(`div`);t.className=`fo-row`,t.style.cssText=`display:flex;gap:8px;margin-bottom:8px;align-items:center;`,t.innerHTML=`
    ...
    `,document.getElementById(`fo_items`).appendChild(t),t.querySelector(`.fo-name`).addEventListener(`change`,n=>{t.querySelector(`.fo-hint`).textContent=n.target.value?`재고 ${e[n.target.value]||0}판`:``}),t.querySelector(`.fo-qty`).addEventListener(`input`,o),t.querySelector(`.fo-del`).addEventListener(`click`,()=>{t.remove(),o()})}function f(){let e=document.createElement(`div`);e.className=`fo-row fo-virtual`,e.style.cssText=`display:flex;gap:8px;margin-bottom:8px;align-items:center;`,e.innerHTML=`
      ...
      <button class="fo-del btn-secondary" style="font-size:11px;padding:2px 8px;">✕</button>
    `,document.getElementById(`fo_items`).appendChild(e),e.querySelector(`.fo-qty`).addEventListener(`input`,o),e.querySelector(`.fo-del`).addEventListener(`click`,()=>{e.remove(),o()})}document.getElementById(`fo_addStock`).addEventListener(`click`,l),document.getElementById(`fo_addVirtual`).addEventListener(`click`,f),l(),document.getElementById(`fo_save`).addEventListener(`click`,async()=>{let n=document.getElementById(`fo_date`).value;if(!n){alert(`날짜를 입력해주세요.`);return}let o=[],l=!1;if(document.querySelectorAll(`#fo_items .fo-row`).forEach(t=>{let n=t.classList.contains(`fo-virtual`),r=(t.querySelector(`.fo-name`).value||``).trim(),i=parseInt(t.querySelector(`.fo-qty`).value)||0;if(!(!r||i<=0)){if(!n){let t=e[r]||0;i>t&&(alert(`${r}: 재고 ${t}판 / 발주 ${i}판 — 재고 부족`),l=!0)}o.push({productName:r,panCount:i,isVirtual:n})}}),l)return;if(o.length===0){alert(`발주 항목을 추가해주세요.`);return}let d=o.reduce((e,t)=>e+t.panCount,0);if(d>45&&!await u({title:`판수 초과`,message:`총 ${d}판은 45판 기준을 초과합니다. 그래도 저장하시겠습니까?`,confirmText:`저장`}))return;let f;try{f=B(o,t)}catch(e){alert(e.message);return}let p=a(c);f.forEach(e=>{let n=t.find(t=>t.id===e.lotId),i=n&&Number(n.remaining||0)-e.amount<=0;p.update(r(c,`frozenPanLots`,e.lotId),{remaining:s(-e.amount),...i?{closed:!0}:{},updatedAt:new Date})});let m=r(i(c,`freezeOrders`));p.set(m,{date:n,items:o,totalPans:d,deductions:f,status:`active`,layout:null,layoutConfirmedAt:null,layoutConfirmedBy:null,createdAt:new Date,createdBy:x()||``}),await p.commit(),closeModal(),await O(),alert(`발주서 저장 완료! 동결판 재고가 차감되었습니다.`)})}function M(e,t){let n=(e.items||[]).reduce((e,t)=>e+Number(t.panCount||0),0),r=(e.items||[]).map(e=>`
    <tr>
  ...
  `,!0,`1060px`),I(),L(),document.getElementById(`bp_print`).addEventListener(`click`,()=>H(e,h,v)),document.getElementById(`bp_reset`).addEventListener(`click`,async()=>{await u({title:`초기화`,message:`배치를 모두 초기화할까요?`,confirmText:`초기화`})&&(h=Array(p).fill(null),g=null,I(),L())}),document.getElementById(`bp_confirm`).addEventListener(`click`,async()=>{let t=h.filter(Boolean).length;if(t===0){alert(`배치된 항목이 없습니다.`);return}await u({title:`배치 확정`,message:`${t}/45칸 배치 상태로 확정하시겠습니까?`,confirmText:`확정`})&&(await n(r(c,`freezeOrders`,e.id),{layout:h,layoutConfirmedAt:new Date,layoutConfirmedBy:x()||``,updatedAt:new Date}),closeModal(),await O(),alert(`배치 확정 완료!`))})}function I(){let e=document.getElementById(`bp_products`);e&&(e.innerHTML=(_.items||[]).map(e=>{let t=v[e.productName]||`#ccc`,n=h.filter(t=>t===e.productName).length,r=e.panCount-n,i=g===e.productName;return`
