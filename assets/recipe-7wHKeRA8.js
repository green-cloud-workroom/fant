import{_ as e,d as t,g as n,i as r,o as i,u as a,v as o,w as s,y as c}from"./index.esm-ESivpAya.js";import{n as l}from"./firebase-I0rBIq9V.js";import{B as u,M as d,g as f,z as p}from"./index-BMnUILA5.js";import{n as m}from"./activityLogs-Dl6VA3lF.js";import{t as h}from"./sortable-BlBt-ybB.js";import{n as g,r as _,t as v}from"./supplement-ImEXygnG.js";var y=[],b=null,x=[],S=[],C=[],w=[],T=[{methodKey:`rotary`,label:`로터리`},{methodKey:`manual`,label:`수동`}];async function ee(){return(await i(t(o(l,`meatTypes`),a(`sortOrder`)))).docs.map(e=>({id:e.id,...e.data()}))}async function te(){return(await i(t(o(l,`bagTypes`),a(`sortOrder`)))).docs.map(e=>({id:e.id,...e.data()}))}async function ne(){let e=document.getElementById(`mainContent`);e.innerHTML=`<div style="padding:24px;"><p>레시피 로딩 중...</p></div>`;let t=await Promise.all([E(),ee(),te()]);e.isConnected&&([y,x,S]=t,D())}async function E(){return(await i(t(o(l,`recipes`),a(`sortOrder`)))).docs.map(e=>({id:e.id,...e.data()}))}function D(){let e=document.getElementById(`mainContent`);e.innerHTML=`
  ...
  `,W(),G(),document.getElementById(`btnNewRecipe`)?.addEventListener(`click`,q)}function O(e){let t=u===`admin`||u===`office`;return`
  ...
  `}function P(){return w.length===0?`<div class="production-methods-empty">등록된 생산 방식별 환산값이 없습니다.</div>`:`
    ...
    `).join(``)}
  `}function F(){let e=document.getElementById(`productionMethodsTable`);e&&(e.innerHTML=P());let t=document.getElementById(`productionMethodAddSelect`),n=document.getElementById(`btnAddProductionMethod`),r=document.getElementById(`productionMethodsHint`);if(!t||!n||!r)return;let i=new Set(w.map(e=>e.methodKey)),a=T.filter(e=>!i.has(e.methodKey));t.innerHTML=a.map(e=>`<option value="${e.methodKey}">${e.label}</option>`).join(``),t.disabled=a.length===0,n.disabled=a.length===0,r.textContent=a.length===0?`더 추가할 방식이 없습니다`:``}function I(){F(),document.getElementById(`btnAddProductionMethod`)?.addEventListener(`click`,()=>{let e=document.getElementById(`productionMethodAddSelect`)?.value;!e||w.some(t=>t.methodKey===e)||(w.push({methodKey:e,label:j(e),unitToBox:0,effectiveDate:d(),active:!0}),F())})}function L(){let e=Array.from(document.querySelectorAll(`.production-method-row`)),t=new Set,n=[];for(let r of e){let e=r.dataset.methodKey;if(!e||t.has(e))return alert(`같은 생산방식은 중복 등록할 수 없습니다.`),null;t.add(e);let i=Number(r.querySelector(`.production-method-unit`)?.value),a=r.querySelector(`.production-method-date`)?.value||``;if(!Number.isFinite(i)||i<=0)return alert(`환산값은 0보다 큰 숫자로 입력해주세요.`),null;if(!a)return alert(`환산값 적용 시작일을 입력해주세요.`),null;n.push({methodKey:e,label:j(e),unitToBox:i,effectiveDate:a,active:r.querySelector(`.production-method-active-input`)?.checked!==!1})}return n}function R(e,t){let n=new Map(M(e).map(e=>[e.methodKey,e]));return t.map(e=>{let t=n.get(e.methodKey);return t?Number(t.unitToBox)===Number(e.unitToBox)?null:{methodKey:e.methodKey,from:Number(t.unitToBox),to:e.unitToBox,effectiveDate:e.effectiveDate}:{methodKey:e.methodKey,from:null,to:e.unitToBox,effectiveDate:e.effectiveDate}}).filter(Boolean)}function z(e,t,n){n.forEach(n=>{let r=c(o(l,`recipes`,t,`conversionHistory`));e.set(r,{methodKey:n.methodKey,unitToBox:n.to,prevUnitToBox:n.from,effectiveDate:n.effectiveDate,reason:`manual`,basedOnAvgOfRecent5:!1,createdAt:s(),createdBy:p?.uid||null})})}function B(e,t,n,r){let i=t.displayName||k(t);return{recipeId:e,recipeName:i,unit:n,name:g(i,n),active:t.active!==!1,sortOrder:_(t.sortOrder,r),updatedAt:new Date,updatedBy:p?.uid||null}}async function V(e){let[a,s,u]=await Promise.all([i(t(o(l,`productions`),n(`recipeId`,`==`,e.id))),i(t(o(l,`schedules`),n(`recipeId`,`==`,e.id))),i(t(o(l,`supplementTypes`),n(`recipeId`,`==`,e.id)))]),d=[];for(let e of u.docs){let a={id:e.id,...e.data()},[s,u]=await Promise.all([r(c(l,`supplementStock`,a.id)),i(t(o(l,`supplementLogs`),n(`supplementTypeId`,`==`,a.id)))]),f=Number(s.data()?.currentQty||0);d.push({sku:a,stockQty:f,stockExists:s.exists(),logDocs:u.docs})}return{productionCount:a.size,scheduleCount:s.size,supplementSummaries:d}}async function H(e,a){let s=[];for(let u of a){let a=v(e.id,u),d=await r(c(l,`supplementStock`,a)),f=await i(t(o(l,`supplementLogs`),n(`supplementTypeId`,`==`,a))),p=Number(d.data()?.currentQty||0);s.push({unit:u,supplementTypeId:a,name:g(k(e),u),stockQty:p,logCount:f.size,logDocs:f.docs})}return s}async function U(e,t){if(t.length===0)return!0;let n=await H(e,t);if(n.some(e=>e.stockQty>0)&&typeof window.openBlockingModal==`function`)return await window.openBlockingModal({variant:`warning`,data:{date:d(),warnings:n.map(e=>({reason:e.stockQty>0?`영양제 ${e.name} ${e.stockQty}봉이 있는데 정말 삭제하시겠습니까?`:`영양제 ${e.name}을 삭제합니다.`,details:[`생산단위: ${e.unit}`,`현재 재고: ${e.stockQty}봉`,`차감 이력: ${e.logCount}건`,`영양제 SKU, 재고, 이력이 함께 삭제됩니다.`]}))}})?n:!1;let r=n.some(e=>e.logCount>0)?n.map(e=>e.logCount>0?`생산단위 ${e.unit}의 차감 이력이 ${e.logCount}건 있습니다. 삭제 시 이력도 함께 삭제됩니다.`:`생산단위 ${e.unit}을 프리셋에서 삭제합니다. 영양제 SKU도 함께 삭제됩니다.`).join(`
`):n.map(e=>`생산단위 ${e.unit}을 프리셋에서 삭제합니다. 영양제 SKU도 함께 삭제됩니다.`).join(`
`);return window.confirm(`${r}\n진행하시겠습니까?`)?n:!1}function W(){document.querySelectorAll(`.recipe-list-item`).forEach(e=>{e.addEventListener(`click`,t=>{if(t.target.closest(`.toggle-switch`)||t.target.closest(`.drag-handle`))return;let n=e.dataset.id;b=n,J(y.find(e=>e.id===n)),document.querySelectorAll(`.recipe-list-item`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`)})}),document.querySelectorAll(`.recipe-active-toggle`).forEach(r=>{r.addEventListener(`change`,async r=>{if(u!==`admin`&&u!==`office`)return;let a=r.target.dataset.id,s=r.target.checked,f=y.find(e=>e.id===a),h=f?.active!==!1;try{let r=e(l);r.update(c(l,`recipes`,a),{active:s,updatedAt:new Date}),(await i(t(o(l,`supplementTypes`),n(`recipeId`,`==`,a)))).docs.forEach(e=>{r.update(c(l,`supplementTypes`,e.id),{active:s,updatedAt:new Date,updatedBy:p?.uid||null})}),await r.commit(),f&&(f.active=s),h!==s&&await m({action:`recipe`,subAction:`activeToggle`,date:d(),staff:A(),message:`Recipe ${s?`active`:`inactive`} — ${f?k(f):a}`,details:{recipeId:a,recipeName:f?k(f):null,active:s}})}catch(e){console.error(`[recipe] active save failed:`,e),alert(`활성 상태 저장 중 오류가 발생했습니다.`),r.target.checked=!s}})})}function G(){u!==`admin`&&u!==`office`||[[`recipeListRaw`,`raw`],[`recipeListFreezeDry`,`freezeDry`]].forEach(([e,t])=>{let n=document.getElementById(e);n&&h.create(n,{handle:`.drag-handle`,animation:150,ghostClass:`sortable-ghost`,chosenClass:`sortable-chosen`,onEnd:async e=>{e.oldIndex!==e.newIndex&&await K(t)}})})}async function K(r){let a=Array.from(document.getElementById(`recipeListRaw`)?.querySelectorAll(`.recipe-list-item`)||[]),s=Array.from(document.getElementById(`recipeListFreezeDry`)?.querySelectorAll(`.recipe-list-item`)||[]),u=[...a.map((e,t)=>({id:e.dataset.id,sortOrder:t})),...s.map((e,t)=>({id:e.dataset.id,sortOrder:a.length+t}))].filter(e=>e.id);if(u.length===0)return;if(u.length>450){alert(`순번 저장 항목이 너무 많습니다. 관리자에게 문의해주세요.`);return}let d=new Date,f=e(l),m=new Map(u.map(e=>[e.id,e.sortOrder])),h=y.filter(e=>m.has(e.id)),g=await Promise.all(h.map(async e=>({recipe:e,snap:await i(t(o(l,`supplementTypes`),n(`recipeId`,`==`,e.id)))}))),v=0;if(u.forEach(({id:e,sortOrder:t})=>{f.update(c(l,`recipes`,e),{sortOrder:t,updatedAt:d}),v+=1}),g.forEach(({recipe:e,snap:t})=>{let n=m.get(e.id),r=Array.isArray(e.unitPresets)?e.unitPresets:[];t.docs.forEach(e=>{let t=r.indexOf(e.data().unit);t<0||(f.update(c(l,`supplementTypes`,e.id),{sortOrder:_(n,t),updatedAt:d,updatedBy:p?.uid||null}),v+=1)})}),v>500){alert(`순번 저장 항목이 너무 많습니다. 관리자에게 문의해주세요.`);return}try{await f.commit(),y=y.map(e=>m.has(e.id)?{...e,sortOrder:m.get(e.id),updatedAt:d}:e).sort((e,t)=>(e.sortOrder??0)-(t.sortOrder??0))}catch(e){if(console.error(`[recipe] reorder save failed:`,e),alert(`순번 저장 실패: `+(e.message||e)),y=await E(),D(),b){let e=y.find(e=>e.id===b);e&&J(e)}}}function q(){b=null,document.querySelectorAll(`.recipe-list-item`).forEach(e=>e.classList.remove(`active`)),J(null)}function J(e){let t=document.getElementById(`recipeDetail`),n=!e,r=u===`admin`||u===`office`;C=Array.isArray(e?.unitPresets)?[...e.unitPresets]:[],w=M(e?.productionMethods),t.innerHTML=`
...
`)}\n\n레시피와 영양제 SKU ${n}개, 영양제 이력 ${r}건이 모두 삭제됩니다.\n진행하시겠습니까?`:r>0?`영양제 이력 ${r}건이 함께 삭제됩니다.\n레시피와 영양제 SKU ${n}개를 삭제하시겠습니까?`:n>0?`레시피와 영양제 SKU ${n}개를 삭제하시겠습니까?`:`${k(e)} 레시피를 삭제하시겠습니까?`}async function oe(t){if(t?.id){if(u!==`admin`&&u!==`office`){alert(`레시피 삭제는 대표/사무실 계정만 가능합니다.`);return}try{let n=await V(t);if(n.productionCount>0){alert(`이 레시피로 만든 생산 기록이 ${n.productionCount}건 있어 삭제할 수 없습니다.\n비활성화로 처리해주세요.`);return}if(n.scheduleCount>0){alert(`입고 예정 ${n.scheduleCount}건이 있어 삭제할 수 없습니다.`);return}if(!await f({title:`레시피 삭제`,message:ae(t,n.supplementSummaries),confirmText:`삭제`,danger:!0}))return;let r=e(l);r.delete(c(l,`recipes`,t.id)),n.supplementSummaries.forEach(e=>{r.delete(c(l,`supplementTypes`,e.sku.id)),e.stockExists&&r.delete(c(l,`supplementStock`,e.sku.id)),e.logDocs.forEach(e=>{r.delete(c(l,`supplementLogs`,e.id))})}),await r.commit();let i=n.supplementSummaries.length,a=n.supplementSummaries.reduce((e,t)=>e+t.stockQty,0),o=n.supplementSummaries.reduce((e,t)=>e+t.logDocs.length,0);await m({action:`recipe`,subAction:`delete`,date:d(),staff:A(),message:`레시피 삭제 — ${k(t)}`,details:{recipeId:t.id,recipeName:k(t),category:t.category===`freezeDry`?`dried`:`raw`,supplementSkuCount:i,supplementTotalQty:a,supplementLogCount:o}}),b=null,y=await E(),D(),alert(`레시피가 삭제되었습니다.`)}catch(e){console.error(`[recipe] delete failed:`,e),alert(`레시피 삭제 중 오류가 발생했습니다.`)}}}function Y(){return C.length?C.map((e,t)=>`
...
`),r=document.getElementById(`ingredientBody`),i=e.target.closest(`tr`),a=i?[...r.querySelectorAll(`tr`)].indexOf(i):0;n.forEach((e,t)=>{let n=e.split(`	`),i=n[0]?.trim()||``,o=n[1]?.trim()||``,s=x.find(e=>e.active!==!1&&e.name===i),c=!!s,l=s?.id||null,u=r.querySelectorAll(`tr`),d=a+t;if(u[d]){let e=u[d];e.querySelector(`.ing-name`).value=i,e.querySelector(`.ing-weight`).value=o;let t=e.querySelector(`.ing-weight-unit`);t&&(t.value=`g`,t.dataset.previousUnit=`g`);let n=e.querySelector(`.ing-linked`),r=e.querySelector(`.ing-meat-type`);n.checked=c,c?(r.style.display=``,r.value=l):(r.style.display=`none`,r.value=``)}else{let e=r.querySelectorAll(`tr`).length;r.insertAdjacentHTML(`beforeend`,Q({name:i,baseWeightG:o,weightDisplayUnit:`g`,linkedToInventory:c,meatTypeId:l},e)),$()}})}function de(){let e=document.querySelectorAll(`#ingredientBody tr`);return Array.from(e).map((e,t)=>{let n=e.querySelector(`.ing-linked`).checked,r=n&&e.querySelector(`.ing-meat-type`).value||null,i=e.querySelector(`.ing-weight-unit`)?.value===`kg`?`kg`:`g`,a=(parseFloat(e.querySelector(`.ing-weight`).value)||0)*(i===`kg`?1e3:1);return{id:Date.now().toString()+t,name:e.querySelector(`.ing-name`).value.trim(),baseWeightG:a,weightDisplayUnit:i,isProductionUnit:e.querySelector(`.ing-unit-radio`).checked,unitName:e.querySelector(`.ing-unit-name`).value.trim(),autoDeductInventory:n,linkedToInventory:n,meatTypeId:r,sortOrder:t}}).filter(e=>e.name)}async function fe(t){if(u!==`admin`&&u!==`office`){alert(`레시피 저장은 대표/사무실 계정만 가능합니다.`);return}let n=document.getElementById(`recipeName`).value.trim(),i=document.getElementById(`recipeCategory`).value,a=document.getElementById(`recipeTarget`).value,s=t?y.find(e=>e.id===t):null,f=Array.isArray(s?.unitPresets)?s.unitPresets:[],h=M(s?.productionMethods);if(!n||!i||!a){alert(`레시피명, 카테고리, 대상은 필수입니다.`);return}let g=document.getElementById(`recipeUsesSupplement`)?.checked??!0,_={name:n,displayName:(a===`cat`?`고양이 `:a===`dog`?`강아지 `:``)+n,category:i,target:a,color:document.getElementById(`recipeColor`).value,note:document.getElementById(`recipeNote`).value.trim(),active:t?s?.active!==!1:!0,sortOrder:t?s?.sortOrder??y.length:y.length,ingredients:de(),unitPresets:[...C],usesSupplement:g,version:1,updatedAt:new Date},x=g?_.unitPresets:[];if(i===`raw`){_.packWeightG=parseFloat(document.getElementById(`packWeightG`).value)||null,_.packsPerPlate=parseInt(document.getElementById(`recipePacksPerPlate`)?.value,10)||null;let e=document.getElementById(`recipeBagType`)?.value||``;if(!e){alert(`생식 레시피는 사용 봉투를 선택해야 합니다.`);return}_.bagTypeId=e;let t=L();if(t===null)return;_.productionMethods=t}else _.productionMethods=[];if(i===`freezeDry`){let e=document.getElementById(`requiresSeparation`).value===`true`;_.freezeDryBagCountPerUnit=parseFloat(document.getElementById(`freezeDryBagCount`).value)||null,_.breadPanCountPerUnit=e?parseFloat(document.getElementById(`breadPanCount`).value)||null:0,_.freezePanCountPerUnit=parseFloat(document.getElementById(`freezePanCount`).value)||null,_.requiresSeparation=e}let S=i===`raw`?R(h,_.productionMethods):[];try{if(t){let n=f.filter(e=>!x.includes(e)),i=await U({id:t,...s},n);if(i===!1){C=[...f],X();return}let a=await Promise.all(x.map(async e=>{let n=v(t,e),[i,a]=await Promise.all([r(c(l,`supplementTypes`,n)),r(c(l,`supplementStock`,n))]);return{unit:e,supplementTypeId:n,hasType:i.exists(),hasStock:a.exists()}})),o=new Map(a.map(e=>[e.unit,e])),u=e(l);u.update(c(l,`recipes`,t),_),z(u,t,S);let d=new Map((Array.isArray(i)?i:[]).map(e=>[e.unit,e]));x.forEach((e,n)=>{let r=v(t,e),i=B(t,_,e,n),a=o.get(e);f.includes(e)&&a?.hasType?u.update(c(l,`supplementTypes`,r),i):u.set(c(l,`supplementTypes`,r),{id:r,...i,createdAt:new Date,createdBy:p?.uid||null}),(!f.includes(e)||!a?.hasStock)&&u.set(c(l,`supplementStock`,r),{id:r,supplementTypeId:r,currentQty:0,updatedAt:new Date})}),n.forEach(e=>{let n=d.get(e),r=v(t,e);u.delete(c(l,`supplementTypes`,r)),u.delete(c(l,`supplementStock`,r)),(n?.logDocs||[]).forEach(e=>{u.delete(c(l,`supplementLogs`,e.id))})}),await u.commit(),b=t}else{_.createdAt=new Date,_.createdBy=p?.uid||null,_.updatedBy=p?.uid||null;let t=c(o(l,`recipes`)),n=e(l);n.set(t,_),z(n,t.id,S),x.forEach((e,r)=>{let i=v(t.id,e),a=B(t.id,_,e,r);n.set(c(l,`supplementTypes`,i),{id:i,...a,createdAt:new Date,createdBy:p?.uid||null}),n.set(c(l,`supplementStock`,i),{id:i,supplementTypeId:i,currentQty:0,updatedAt:new Date})}),await n.commit(),b=t.id}}catch(e){console.error(`[recipe] save failed:`,e),alert(`레시피 저장 중 오류가 발생했습니다.`);return}if(S.length>0)try{await m({action:`conversion`,subAction:`manualEdit`,date:d(),staff:A(),message:`환산값 변경 - ${_.displayName}`,details:{recipeId:b,recipeName:_.displayName,changes:S}})}catch(e){console.error(`[recipe] conversion activity log failed:`,e)}if(y=await E(),D(),b){let e=y.find(e=>e.id===b);e&&(J(e),document.querySelector(`[data-id="${b}"]`)?.classList.add(`active`))}alert(`저장되었습니다.`)}export{ne as renderRecipe};