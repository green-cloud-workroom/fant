import{b as e,d as t,g as n,s as r,u as i,y as a}from"./index.esm-rHmxwfvm.js";import{n as o}from"./firebase-qGjqjNvO.js";import{G as s,L as c,a as l,c as u,d,h as f,l as p,m,o as h,p as g,s as _,u as v,x as y}from"./index-DRMzawyA.js";import{n as b}from"./activityLogs-DYC0UZD3.js";import{t as ee}from"./sortable-Du2optuH.js";import{t as x}from"./pageResources-BE3e2wtt.js";import{t as S}from"./readCommand-zTaCVxIR.js";import{t as te}from"./pageRefresh-D3ZSodaR.js";import{t as C}from"./commandBatch-BAK3Fgth.js";var w=x(`equipment`);async function T(e,t=[`admin`,`office`]){let n=y();try{return await S(w,t=>(t.isCurrent=()=>!n||n.isCurrent(),e(t)),{roles:t})}catch(e){console.error(`[설비 저장]`,e),alert(e.message)}}var E=`__all__`,D=[],O=[],k=E,A=null,j=!1,M=!1,N=[];async function ne(e={}){let t=document.getElementById(`mainContent`);t.innerHTML=`<div style="padding:24px;"><p>설비 부품 로딩 중...</p></div>`,await F(e),document.getElementById(`mainContent`)===t&&R()}async function P(e={getDocs:r}){return(await e.getDocs(t(a(o,`equipments`),i(`sortOrder`)))).docs.map(e=>({id:e.id,...e.data()}))}async function F({force:t=!1}={}){let n=document.getElementById(`mainContent`),r=await w.load(async t=>{let n=[`senior`,`lead`,`office`],[r,i,...a]=await Promise.all([P(t),d(t),...n.map(n=>t.getDoc(e(o,`staffGroups`,n)))]);return{equipment:r,parts:i,staff:Object.fromEntries(n.map((e,t)=>[e,a[t].exists()&&a[t].data().members||[]]))}},{force:t,onChange:te(w,ne)});!r||document.getElementById(`mainContent`)!==n||(D=r.equipment,O=r.parts,we=r.staff)}function I(){return s===`admin`||s===`office`}function L(e){return String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e])}function R(){let e=document.getElementById(`mainContent`);e.innerHTML=`
  ...
  `}function ue(e,t){let n=c(),r=O.filter(e=>e.equipmentId===t.id),i=r.length;j&&(r=r.filter(e=>{let t=v(e,n);return t.overdue||t.dueSoon})),M&&(r=r.filter(e=>v(e,n).low)),r=le(r);let a=t.active===!1;e.innerHTML=`
  ...
  `,K(),A&&q()}function K(){document.querySelectorAll(`.eq-part-row`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`.eq-act`)||(A=A===e.dataset.partId?null:e.dataset.partId,document.querySelectorAll(`.eq-part-row`).forEach(e=>e.classList.toggle(`eq-part-row-selected`,e.dataset.partId===A)),q())})}),document.querySelectorAll(`.eq-act`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=O.find(t=>t.id===e.dataset.partId);if(!n)return;let r=e.dataset.act;r===`replace`?be(n):r===`in`?xe(n):r===`adjust`?Se(n):r===`edit`&&_e(n,D.find(e=>e.id===n.equipmentId))})})}var fe={replace:`교체`,in:`입고`,adjust:`조정`};async function q(){let e=document.getElementById(`partHistory`);if(!e)return;let i=O.find(e=>e.id===A);if(!i){e.innerHTML=``;return}e.innerHTML=`<p style="font-size:12px;color:#888;padding:8px 0;">이력 로딩 중...</p>`;let s=[];try{s=(await r(t(a(o,`equipmentPartLogs`),n(`partId`,`==`,i.id)))).docs.map(e=>({id:e.id,...e.data()})).sort((e,t)=>(t.timestamp?.toMillis?.()||0)-(e.timestamp?.toMillis?.()||0)).slice(0,50)}catch(e){console.error(`[equipment] history load failed:`,e)}document.getElementById(`partHistory`)!==e||A!==i.id||(e.innerHTML=`
    <div class="form-section">
      <div class="section-header">
        <span class="section-title">${L(m(i))} · 이력</span>
    ...
    ${n&&i.length>0?`
      <div class="form-group">
        ...
        <select id="m_copyFrom">
          ...
          ${i.map(e=>`<option value="${e.id}">${L(g(e.category,e.alias))}</option>`).join(``)}
        </select>
  ...
  `),document.getElementById(`btnModalCancel`).addEventListener(`click`,$),document.getElementById(`btnSaveCategories`).addEventListener(`click`,()=>T(async t=>{let n=C(t,o),{batch:r,addDoc:i,updateDoc:a,recordActivity:s}=n,l=[];if(document.querySelectorAll(`.m_catRename`).forEach(e=>{let t=e.dataset.original,n=e.value.trim();n&&n!==t&&l.push({from:t,to:n})}),l.length===0){$();return}let u=new Date;try{if(l.forEach(({from:t,to:n})=>{D.filter(e=>e.category===t).forEach(t=>r.update(e(o,`equipments`,t.id),{category:n,updatedAt:u})),O.filter(e=>e.equipmentCategory===t).forEach(t=>r.update(e(o,`equipmentParts`,t.id),{equipmentCategory:n,updatedAt:u}))}),await s({action:`equipment`,subAction:`renameCategory`,date:c(),staff:Z(),message:`기계 종류 이름 변경 — ${l.map(e=>`${e.from}→${e.to}`).join(`, `)}`,details:{changes:l}}),await n.commit(),!t.isCurrent()||($(),await F({force:!0}),!t.isCurrent()))return;R()}catch(e){console.error(`[equipment] rename category failed:`,e),alert(e.message||`저장 중 오류가 발생했습니다.`)}}))}function _e(t,n){if(!I()||!n)return;let r=!t;Q(`
    <h3 class="modal-title">${r?`부품 추가`:`부품 수정`} — ${L(g(n.category,n.alias))}</h3>
    <div class="form-row">
      <div class="form-group">
        <label>부품명 *</label>
  ...
  `),document.getElementById(`btnModalCancel`).addEventListener(`click`,$),document.getElementById(`btnDeletePart`)?.addEventListener(`click`,()=>ve(t)),document.getElementById(`btnSavePart`).addEventListener(`click`,()=>T(async i=>{let s=C(i,o),{batch:l,addDoc:u,updateDoc:d,recordActivity:f}=s,p=document.getElementById(`m_name`).value.trim(),m=document.getElementById(`m_spec`).value.trim(),_=parseInt(document.getElementById(`m_cycleValue`).value)||0,v=document.getElementById(`m_cycleUnit`).value,y=document.getElementById(`m_lastReplacedAt`).value||null,b=Math.max(0,parseInt(document.getElementById(`m_minimumQty`).value)||0),ee=document.getElementById(`m_partMemo`).value.trim();if(!p){alert(`부품명은 필수입니다.`);return}if(_<=0){alert(`교체 주기를 입력해주세요.`);return}let x=new Date,S={name:p,spec:m,cycleValue:_,cycleUnit:v,lastReplacedAt:y,minimumQty:b,memo:ee,nextDueAt:h(y,_,v),updatedAt:x};try{if(r){let e=Math.max(0,parseInt(document.getElementById(`m_currentQty`).value)||0),t=await u(a(o,`equipmentParts`),{...S,equipmentId:n.id,equipmentCategory:n.category,equipmentAlias:n.alias,currentQty:e,active:!0,sortOrder:O.filter(e=>e.equipmentId===n.id).length,createdAt:x});await f({action:`equipment`,subAction:`partCreate`,date:c(),staff:Z(),message:`부품 추가 — ${g(n.category,n.alias)} · ${p} (재고 ${e})`,details:{partId:t.id,equipmentId:n.id,name:p,cycleValue:_,cycleUnit:v,currentQty:e,minimumQty:b}})}else S.active=document.getElementById(`m_partActive`).checked,await d(e(o,`equipmentParts`,t.id),S),await f({action:`equipment`,subAction:`partUpdate`,date:c(),staff:Z(),message:`부품 수정 — ${g(n.category,n.alias)} · ${p}${S.active?``:` (비활성)`}`,details:{partId:t.id,equipmentId:n.id,name:p,cycleValue:_,cycleUnit:v,minimumQty:b,active:S.active}});if(await s.commit(),!i.isCurrent()||($(),await F({force:!0}),!i.isCurrent()))return;z(),U()}catch(e){console.error(`[equipment] part save failed:`,e),alert(`저장 중 오류가 발생했습니다: `+(e.message||e))}}))}async function ve(e){return T(t=>ye(e,t))}async function ye(r,i){let s=C(i,o),{batch:l,recordActivity:u}=s,{getDocs:d}=i;if(!I())return;let p=[];try{p=(await d(t(a(o,`equipmentPartLogs`),n(`partId`,`==`,r.id)))).docs}catch(e){throw e}if(await f({title:`부품 삭제`,message:`${m(r)}을(를) 삭제하시겠습니까?${p.length?`\n이력 ${p.length}건이 함께 삭제됩니다.`:``}`,confirmText:`삭제`,danger:!0}))try{if(l.delete(e(o,`equipmentParts`,r.id)),p.forEach(t=>l.delete(e(o,`equipmentPartLogs`,t.id))),await u({action:`equipment`,subAction:`partDelete`,date:c(),staff:Z(),message:`부품 삭제 — ${m(r)}`,details:{partId:r.id,equipmentId:r.equipmentId,name:r.name,logCount:p.length}}),await s.commit(),!i.isCurrent()||($(),A===r.id&&(A=null),await F({force:!0}),!i.isCurrent()))return;z(),U()}catch(e){console.error(`[equipment] part delete failed:`,e),alert(e.message||`삭제 중 오류가 발생했습니다.`)}}function Y(e){return`
    <select id="${e}">
      <option value="">선택</option>
      ${Te([`senior`,`lead`,`office`])}
    </select>
  `}function be(e){let t=Number(e.currentQty||0);Q(`
    <h3 class="modal-title">부품 교체 — ${L(m(e))}</h3>
  ...
  `),document.getElementById(`btnModalCancel`).addEventListener(`click`,$),document.getElementById(`btnSaveReplace`).addEventListener(`click`,async()=>{let n=document.getElementById(`m_date`).value,r=document.getElementById(`m_staff`).value,i=document.getElementById(`m_note`).value.trim();if(!n||!r){alert(`교체일과 담당자는 필수입니다.`);return}let a=t,o=a>0?-1:0,s=a+o,c=h(n,e.cycleValue,e.cycleUnit);await X(e,{type:`replace`,qty:o,before:a,after:s,date:n,staff:r,note:i,partPatch:{currentQty:s,lastReplacedAt:n,nextDueAt:c},message:`부품 교체 — ${m(e)} (재고 ${a}→${s}, 다음 ${c||`-`}) / 담당: ${r}`})})}function xe(e){Q(`
    <h3 class="modal-title">부품 입고 — ${L(m(e))}</h3>
  ...
  `),document.getElementById(`btnModalCancel`).addEventListener(`click`,$),document.getElementById(`btnSaveIn`).addEventListener(`click`,async()=>{let t=parseInt(document.getElementById(`m_qty`).value),n=document.getElementById(`m_date`).value,r=document.getElementById(`m_staff`).value,i=document.getElementById(`m_note`).value.trim();if(!t||t<=0||!n){alert(`수량과 입고일은 필수입니다.`);return}if(!r){alert(`담당자는 필수입니다.`);return}let a=Number(e.currentQty||0),o=a+t;await X(e,{type:`in`,qty:t,before:a,after:o,date:n,staff:r,note:i,partPatch:{currentQty:o},message:`부품 입고 — ${m(e)} +${t} (재고 ${a}→${o}) / 담당: ${r}`})})}function Se(e){let t=Number(e.currentQty||0);Q(`
    <h3 class="modal-title">재고 조정 — ${L(m(e))}</h3>
