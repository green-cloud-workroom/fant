import{_ as e,b as t,d as n,g as r,s as i,u as a,y as o}from"./index.esm-rHmxwfvm.js";import{n as s}from"./firebase-qGjqjNvO.js";import{n as c}from"./modalManager-Z4b2ihO9.js";import{I as l,W as u,b as d,h as f}from"./index-BvUufXtY.js";import{n as p}from"./activityLogs-CtU4U8Oc.js";import{t as m}from"./closingGuard-C7IU8dK_.js";import{t as h}from"./sortable-B9aQZmIg.js";import{t as g}from"./pageResources-DBecWn5C.js";import{t as _}from"./readCommand-ClLVD-hf.js";import{t as v}from"./pageRefresh-D_VW7uvN.js";var y=g(`bag`);async function b(e,t=[`admin`,`office`]){let n=d();try{return await _(y,t=>(t.isCurrent=()=>!n||n.isCurrent(),e(t)),{roles:t})}catch(e){console.error(`[봉투 저장]`,e),alert(e.message)}}async function x(e,n,r,i,a){return b(async c=>{if(await m(a,c))return!1;let l=t(s,`bagTypes`,e.id),u=t(o(s,`bagLogs`));return await c.transaction(s,async t=>{let a=await t.get(l);if(!a.exists()||Number(a.data().currentQty||0)!==Number(e.currentQty||0))throw Error(`다른 작업으로 봉투 재고가 변경되었습니다. 최신 자료를 다시 확인해주세요.`);t.update(l,n),t.set(u,r),await p(i,{batch:t})},{targets:[l,u]}),!0},[`admin`,`office`,`production`])}var S=[];async function C({force:e=!1}={}){let n=document.getElementById(`mainContent`);n.innerHTML=`<div style="padding:24px;"><p>봉투 재고 로딩 중...</p></div>`;let r=await y.load(async e=>{let n=[`senior`,`lead`,`office`],[r,...i]=await Promise.all([w(e),...n.map(n=>e.getDoc(t(s,`staffGroups`,n)))]);return{bags:r,staff:Object.fromEntries(n.map((e,t)=>[e,i[t].exists()&&i[t].data().members||[]]))}},{force:e,onChange:v(y,C)});!r||!n.isConnected||(S=r.bags,U=r.staff,E())}async function w(e={getDocs:i}){return(await e.getDocs(n(o(s,`bagTypes`),a(`sortOrder`)))).docs.map(e=>({id:e.id,...e.data()}))}var T=null;function E(){let e=document.getElementById(`mainContent`);e.innerHTML=`
    <div class="recipe-wrap">
      <!-- 왼쪽: 봉투 목록 -->
      <div class="recipe-list-panel">
        <div class="panel-header">
          <span class="panel-title">봉투 목록</span>
          ${u===`admin`||u===`office`?`<button class="btn-primary" id="btnNewBag">+ 추가</button>`:``}
        </div>
  ...
  `}function k(){u!==`admin`&&u!==`office`||[[`bagListRaw`,`raw`],[`bagListFreezeDry`,`freezeDry`]].forEach(([e,t])=>{let n=document.getElementById(e);n&&h.create(n,{handle:`.drag-handle`,animation:150,ghostClass:`sortable-ghost`,chosenClass:`sortable-chosen`,onEnd:async e=>{e.oldIndex!==e.newIndex&&await A(t)}})})}async function A(e){return b(t=>j(e,t))}async function j(n,r){let i=document.getElementById(n===`raw`?`bagListRaw`:`bagListFreezeDry`);if(!i)return;let a=Array.from(i.querySelectorAll(`.recipe-list-item`)).map(e=>e.dataset.id).filter(Boolean),o=S.filter(e=>e.category===`raw`).length,c=n===`freezeDry`?o:0,l=new Date,u=e(s);a.forEach((e,n)=>{u.update(t(s,`bagTypes`,e),{sortOrder:c+n,updatedAt:l})});try{if(await r.commit(u,{targets:a.map(e=>t(s,`bagTypes`,e))}),!r.isCurrent())return;let e=new Map(a.map((e,t)=>[e,c+t]));S=S.map(t=>e.has(t.id)?{...t,sortOrder:e.get(t.id),updatedAt:l}:t).sort((e,t)=>(e.sortOrder??0)-(t.sortOrder??0)),await C({force:!0})}catch(e){console.error(`[bag] reorder save failed:`,e),alert(`순번 저장 실패: `+(e.message||e));let t=await w();if(!r.isCurrent())return;if(S=t,E(),T){let e=S.find(e=>e.id===T);e&&await N(e)}}}function M(){document.querySelectorAll(`.recipe-list-item`).forEach(e=>{e.addEventListener(`click`,t=>{t.target.closest(`.drag-handle`)||(T=e.dataset.id,N(S.find(e=>e.id===T)),document.querySelectorAll(`.recipe-list-item`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`))})}),document.querySelectorAll(`.bag-active-toggle`).forEach(n=>{n.addEventListener(`click`,e=>e.stopPropagation()),n.addEventListener(`change`,async n=>{if(u!==`admin`&&u!==`office`){alert(`봉투 종류 활성 변경은 대표/사무실 계정만 가능합니다.`),n.target.checked=!n.target.checked;return}let r=n.target.dataset.id,i=n.target.checked,a=S.find(e=>e.id===r),o=a?.active!==!1;try{await _(y,async n=>{let c=d(),u=e(s);if(u.update(t(s,`bagTypes`,r),{active:i,updatedAt:new Date}),o!==i&&await p({action:`bag`,subAction:`activeToggle`,date:l(),staff:K(),message:`봉투 종류 ${i?`활성`:`비활성`} — ${a?.name||r}`,details:{bagTypeId:r,bagName:a?.name||null,active:i}},{batch:u}),await n.commit(u,{targets:[t(s,`bagTypes`,r)]}),a&&(a.active=i),!(c&&!c.isCurrent())&&(await C({force:!0}),T)){let e=S.find(e=>e.id===T);e&&await N(e)}})}catch(e){console.error(`[bag] active save failed:`,e),alert(e.message||`활성 상태 저장 중 오류가 발생했습니다.`),n.target.checked=!i}})})}async function N(e){let t=document.getElementById(`bagDetail`),a=u===`admin`||u===`office`,c=(await i(n(o(s,`bagLogs`),r(`bagTypeId`,`==`,e.id)))).docs.map(e=>({id:e.id,...e.data()})).filter(e=>e.timestamp!==void 0).sort((e,t)=>{let n=e=>e?.toMillis?.()??(e?.seconds?e.seconds*1e3:new Date(e).getTime()||0);return n(t.timestamp)-n(e.timestamp)||t.id.localeCompare(e.id)}).slice(0,30);!t.isConnected||T!==e.id||(t.innerHTML=`
  ...
  `),document.getElementById(`btnSaveBag`).addEventListener(`click`,()=>b(async i=>{let a=e(s);if(u!==`admin`&&u!==`office`){alert(`봉투 등록/수정은 대표/사무실 계정만 가능합니다.`);return}let o=document.getElementById(`m_bagName`).value.trim(),c=document.getElementById(`m_bagCategory`).value,l=parseInt(document.getElementById(`m_piecesPerBox`).value)||0,d=parseInt(document.getElementById(`m_minBox`).value)||0;if(!o||!c){alert(`봉투명과 종류는 필수입니다.`);return}let f={name:o,category:c,piecesPerBox:l,minimumBoxQty:d,minimumQty:d*l,sortOrder:r?S.length:n.sortOrder,active:r?!0:n.active!==!1,updatedAt:new Date};r?(f.currentQty=0,f.createdAt=new Date,a.set(target,f)):a.update(t(s,`bagTypes`,n.id),f),await i.commit(a,{targets:[target]}),i.isCurrent()&&(closeModal(),await C({force:!0}),alert(r?`봉투 추가 완료!`:`수정 완료!`))}))}function B(e){z(e)}function V(e){G(`
  ...
  `),document.getElementById(`btnSaveAdjust`).addEventListener(`click`,async()=>{let t=d(),n=document.getElementById(`m_adjustType`).value,r=parseInt(document.getElementById(`m_qty`).value),i=document.getElementById(`m_reason`).value.trim(),a=document.getElementById(`m_staff`).value;if(!r||!i||!a){alert(`조정량, 사유, 담당자는 필수입니다.`);return}let o=l();if(await m(o))return;let s=n===`plus`?r:-r,c=e.currentQty||0,u=c+s;if(u<0){alert(`조정 후 잔량이 ${u}장이 됩니다.\n수동조정으로 음수 재고를 만들 수 없습니다.\n현재 ${c}장에서 최대 ${c}장까지만 감소 가능합니다.`);return}let f={currentQty:u,updatedAt:new Date},p={date:l(),timestamp:new Date,bagTypeId:e.id,bagNameSnapshot:e.name,type:`adjust`,qty:s,before:c,after:u,staffName:a,reason:i},h=s>=0?`+`:``;if(!await x(e,f,p,{action:`bag`,subAction:`adjust`,date:o,staff:a,message:`봉투 수동조정 — ${e.name} ${h}${s}장 / 사유: ${i} / 담당: ${a}`,details:{bagTypeId:e.id,bagName:e.name,delta:s,before:c,after:u,reason:i}},o)||t&&!t.isCurrent())return;closeModal(),await C({force:!0});let g=S.find(t=>t.id===e.id);g&&await N(g),alert(`조정 완료!`)})}var U={};function W(e){let t=``;for(let n of e)(U[n]||[]).forEach(e=>{t+=`<option value="${e.name}">${e.name}</option>`});return t}function G(e){let t=document.getElementById(`modalOverlay`);t&&t.remove();let n=document.createElement(`div`);n.id=`modalOverlay`,n.className=`modal-overlay`,n.innerHTML=`<div class="modal-box">${e}</div>`,document.body.appendChild(n),n.addEventListener(`click`,e=>{})}function K(){return u===`admin`?`대표`:u===`office`?`사무실`:u===`production`?`생산실`:`시스템`}c(`bag`,function(){let e=document.getElementById(`modalOverlay`);e&&e.remove()});export{C as renderBag};