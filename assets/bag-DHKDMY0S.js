import{_ as e,b as t,d as n,g as r,s as i,u as a,y as o}from"./index.esm-rHmxwfvm.js";import{n as s}from"./firebase-qGjqjNvO.js";import{n as c}from"./modalManager-Z4b2ihO9.js";import{G as l,L as u,h as d,x as f}from"./index-DRMzawyA.js";import{n as p}from"./activityLogs-DYC0UZD3.js";import{t as m}from"./closingGuard-Dfuw_srM.js";import{t as h}from"./sortable-Du2optuH.js";import{t as g}from"./pageResources-BE3e2wtt.js";import{t as _}from"./readCommand-zTaCVxIR.js";import{t as v}from"./pageRefresh-D3ZSodaR.js";var y=g(`bag`);async function b(e,t=[`admin`,`office`]){let n=f();try{return await _(y,t=>(t.isCurrent=()=>!n||n.isCurrent(),e(t)),{roles:t})}catch(e){console.error(`[봉투 저장]`,e),alert(e.message)}}async function x(e,n,r,i,a){return b(async c=>{if(await m(a,c))return!1;let l=t(s,`bagTypes`,e.id),u=t(o(s,`bagLogs`));return await c.transaction(s,async t=>{let a=await t.get(l);if(!a.exists()||Number(a.data().currentQty||0)!==Number(e.currentQty||0))throw Error(`다른 작업으로 봉투 재고가 변경되었습니다. 최신 자료를 다시 확인해주세요.`);t.update(l,n),t.set(u,r),await p(i,{batch:t})},{targets:[l,u]}),!0},[`admin`,`office`,`production`])}var S=[];async function C({force:e=!1}={}){let n=document.getElementById(`mainContent`);n.innerHTML=`<div style="padding:24px;"><p>봉투 재고 로딩 중...</p></div>`;let r=await y.load(async e=>{let n=[`senior`,`lead`,`office`],[r,...i]=await Promise.all([w(e),...n.map(n=>e.getDoc(t(s,`staffGroups`,n)))]);return{bags:r,staff:Object.fromEntries(n.map((e,t)=>[e,i[t].exists()&&i[t].data().members||[]]))}},{force:e,onChange:v(y,C)});!r||!n.isConnected||(S=r.bags,U=r.staff,E())}async function w(e={getDocs:i}){return(await e.getDocs(n(o(s,`bagTypes`),a(`sortOrder`)))).docs.map(e=>({id:e.id,...e.data()}))}var T=null;function E(){let e=document.getElementById(`mainContent`);e.innerHTML=`
  ...
  `),document.getElementById(`btnSaveAdjust`).addEventListener(`click`,async()=>{let t=f(),n=document.getElementById(`m_adjustType`).value,r=parseInt(document.getElementById(`m_qty`).value),i=document.getElementById(`m_reason`).value.trim(),a=document.getElementById(`m_staff`).value;if(!r||!i||!a){alert(`조정량, 사유, 담당자는 필수입니다.`);return}let o=u();if(await m(o))return;let s=n===`plus`?r:-r,c=e.currentQty||0,l=c+s;if(l<0){alert(`조정 후 잔량이 ${l}장이 됩니다.\n수동조정으로 음수 재고를 만들 수 없습니다.\n현재 ${c}장에서 최대 ${c}장까지만 감소 가능합니다.`);return}let d={currentQty:l,updatedAt:new Date},p={date:u(),timestamp:new Date,bagTypeId:e.id,bagNameSnapshot:e.name,type:`adjust`,qty:s,before:c,after:l,staffName:a,reason:i},h=s>=0?`+`:``;if(!await x(e,d,p,{action:`bag`,subAction:`adjust`,date:o,staff:a,message:`봉투 수동조정 — ${e.name} ${h}${s}장 / 사유: ${i} / 담당: ${a}`,details:{bagTypeId:e.id,bagName:e.name,delta:s,before:c,after:l,reason:i}},o)||t&&!t.isCurrent())return;closeModal(),await C({force:!0});let g=S.find(t=>t.id===e.id);g&&await N(g),alert(`조정 완료!`)})}var U={};function W(e){let t=``;for(let n of e)(U[n]||[]).forEach(e=>{t+=`<option value="${e.name}">${e.name}</option>`});return t}function G(e){let t=document.getElementById(`modalOverlay`);t&&t.remove();let n=document.createElement(`div`);n.id=`modalOverlay`,n.className=`modal-overlay`,n.innerHTML=`<div class="modal-box">${e}</div>`,document.body.appendChild(n),n.addEventListener(`click`,e=>{})}function K(){return l===`admin`?`대표`:l===`office`?`사무실`:l===`production`?`생산실`:`시스템`}c(`bag`,function(){let e=document.getElementById(`modalOverlay`);e&&e.remove()});export{C as renderBag};