import{a as e,b as t,d as n,s as r,u as i,y as a}from"./index.esm-rHmxwfvm.js";import{n as o}from"./firebase-qGjqjNvO.js";import{a as s}from"./formDraft-DoA-Ia7D.js";import{L as c,S as l,m as u}from"./index-D8R-o4n_.js";import{n as d}from"./activityLogs-BSZ1aGr_.js";import{t as f}from"./closingGuard-BOesWeO7.js";import{t as p}from"./readCommand-BqPIk0V7.js";import{t as m}from"./pageRefresh-CF2RoGUC.js";var h=u(`egg`);async function g(e,n,r=null,i=null,s=null){return p(h,async c=>{if(s&&await f(s,c))return!1;let l=t(o,`eggStock`,`global`),u=r?t(a(o,`eggLogs`)):null;return await c.transaction(o,async t=>{let a=await t.get(l),o=a.exists()?a.data():{currentQty:0,minimumQty:0};if(Number(o.currentQty||0)!==Number(e.currentQty||0)||Number(o.minimumQty||0)!==Number(e.minimumQty||0))throw Error(`다른 작업으로 계란 재고가 변경되었습니다. 최신 자료를 다시 확인해주세요.`);t.set(l,n),r&&t.set(u,r),i&&await d(i,{batch:t})},{targets:[l,...u?[u]:[]]}),!0},{roles:[`admin`,`office`,`production`]})}async function _(e){try{return await e()}catch(e){console.error(`[계란 저장]`,e),alert(e.message)}}var v=!1;async function y({force:e=!1}={}){let t=document.getElementById(`mainContent`);t.innerHTML=`<div style="padding:24px;"><p>계란 로딩 중...</p></div>`;let n=await h.load(e=>I(e),{force:e,onChange:m(h,y)});!n||!t.isConnected||(N=n.staff,S(n.stock,n.logs))}async function b(n={getDoc:e}){let r=await n.getDoc(t(o,`eggStock`,`global`));return r.exists()?r.data():{currentQty:0,minimumQty:0}}async function x(e={getDocs:r}){return(await e.getDocs(n(a(o,`eggLogs`),i(`timestamp`,`desc`)))).docs.map(e=>({id:e.id,...e.data()}))}function S(e,t){let n=document.getElementById(`mainContent`),r=Number(e.currentQty||0),i=Number(e.minimumQty||0),a=r<i,o=C(t,r),s=E(t).slice(0,50);n.innerHTML=`
    <div class="page-wrap">
  ...
  `),n||document.getElementById(`m_remaining`).addEventListener(`input`,e=>{let t=parseInt(e.target.value),n=document.getElementById(`m_outPreview`);if(isNaN(t)||t<0)n.textContent=`출고량: -`,n.style.color=`#888`;else{let e=r-t;e<0?(n.textContent=`남은 재고가 현재(${r}개)보다 많습니다`,n.style.color=`#e53e3e`):e===0?(n.textContent=`출고량: 0개 (변동 없음)`,n.style.color=`#888`):(n.textContent=`출고량: ${e}개`,n.style.color=`#2d7a3a`)}}),document.getElementById(`btnSaveEgg`).addEventListener(`click`,()=>_(async()=>{let e=l(),i;if(n){if(i=parseInt(document.getElementById(`m_qty`).value),!i){alert(`수량을 입력해주세요.`);return}}else{let e=parseInt(document.getElementById(`m_remaining`).value);if(isNaN(e)||e<0){alert(`남은 재고를 입력해주세요.`);return}if(e>r){alert(`남은 재고가 현재 재고(${r}개)보다 많을 수 없습니다.`);return}if(i=r-e,i===0){alert(`변동이 없습니다. (출고량 0개)`);return}}let a=document.getElementById(`m_date`).value,o=document.getElementById(`m_staff`).value,s=document.getElementById(`m_note`).value;if(!a){alert(`날짜는 필수입니다.`);return}if(!o){alert(`담당자는 필수입니다.`);return}if(await f(a)||e&&!e.isCurrent())return;let c=n?i:-i,u=t.currentQty,d=u+c;await g(t,{currentQty:d,minimumQty:t.minimumQty,updatedAt:new Date},{date:a,timestamp:new Date,type:n?`in`:`out`,qty:c,before:u,after:d,staffName:o,note:s},{action:`egg`,subAction:n?`in`:`out`,date:a,staff:o,message:`계란 ${n?`입고`:`출고`} — ${n?`+`:`-`}${i}개 / 담당: ${o}`,details:{delta:c,before:u,after:d,note:s||null}},a)&&(e&&!e.isCurrent()||(closeModal(),await y({force:!0}),alert(`${n?`입고`:`출고`} 완료!`)))}))}function j(e){F(`
  ...
  `),document.getElementById(`btnSaveAdjust`).addEventListener(`click`,()=>_(async()=>{let t=l(),n=document.getElementById(`m_adjustType`).value,r=parseInt(document.getElementById(`m_qty`).value),i=document.getElementById(`m_reason`).value.trim(),a=document.getElementById(`m_staff`).value;if(!r||!i||!a){alert(`조정량, 사유, 담당자는 필수입니다.`);return}let o=c();if(await f(o))return;let s=n===`plus`?r:-r,u=e.currentQty,d=u+s;if(d<0){alert(`조정 후 잔량이 ${d}개가 됩니다.\n수동조정으로 음수 재고를 만들 수 없습니다.\n현재 ${u}개에서 최대 ${u}개까지만 감소 가능합니다.`);return}await g(e,{currentQty:d,minimumQty:e.minimumQty,updatedAt:new Date},{date:c(),timestamp:new Date,type:`adjust`,qty:s,before:u,after:d,staffName:a,reason:i},{action:`egg`,subAction:`adjust`,date:o,staff:a,message:`계란 수동조정 — ${s>=0?`+`:``}${s}개 / 사유: ${i} / 담당: ${a}`,details:{delta:s,before:u,after:d,reason:i}},o)&&(t&&!t.isCurrent()||(closeModal(),await y({force:!0}),alert(`조정 완료!`)))}))}function M(e){F(`
