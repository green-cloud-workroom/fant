import{b as e,d as t,s as n,u as r,y as i}from"./index.esm-rHmxwfvm.js";import{n as a}from"./firebase-qGjqjNvO.js";import{n as o}from"./modalManager-Z4b2ihO9.js";import{G as s,L as c,S as l,h as u,x as d}from"./index-DRMzawyA.js";import{t as f}from"./closingGuard-Dfuw_srM.js";import{t as p}from"./pageResources-BE3e2wtt.js";import{t as m}from"./readCommand-zTaCVxIR.js";import{t as h}from"./pageRefresh-D3ZSodaR.js";import{t as g}from"./commandBatch-BAK3Fgth.js";var _=p(`schedule`),v=[],y=[];async function b(e,t=[`admin`,`office`]){let n=d();try{return await m(_,t=>(t.isCurrent=()=>!n||n.isCurrent(),e(t)),{roles:t})}catch(e){console.error(`[입고 예정 저장]`,e),alert(e.message)}}async function x({force:t=!1}={}){let n=document.getElementById(`mainContent`);n.innerHTML=`<div style="padding:24px;"><p>입고 예정관리 로딩 중...</p></div>`;let r=await _.load(async t=>{let n=[`senior`,`lead`,`office`],[r,i,o,...s]=await Promise.all([S(t),C(t),w(t),...n.map(n=>t.getDoc(e(a,`staffGroups`,n)))]);return{schedules:r,meatTypes:i,bagTypes:o,staff:Object.fromEntries(n.map((e,t)=>[e,s[t].exists()&&s[t].data().members||[]]))}},{force:t,onChange:h(_,x)});!r||!n.isConnected||(v=r.meatTypes,y=r.bagTypes,L=r.staff,T(r.schedules))}async function S(e={getDocs:n}){let o=t(i(a,`schedules`),r(`date`,`asc`)),s=(await e.getDocs(o)).docs.map(e=>({id:e.id,...e.data()}));return s.sort((e,t)=>{if(e.date!==t.date)return e.date.localeCompare(t.date);let n=e.createdAt?.toMillis?e.createdAt.toMillis():0;return(t.createdAt?.toMillis?t.createdAt.toMillis():0)-n}),s}async function C(e={getDocs:n}){return(await e.getDocs(i(a,`meatTypes`))).docs.map(e=>({id:e.id,...e.data()}))}async function w(e={getDocs:n}){return(await e.getDocs(i(a,`bagTypes`))).docs.map(e=>({id:e.id,...e.data()}))}function T(e){let t=document.getElementById(`mainContent`),n=c(),r=E(),i=e.filter(e=>e.status===`scheduled`),a=e.filter(e=>e.status!==`scheduled`),o=!1;t.innerHTML=`
    <div class="page-wrap">
  ...
  `),window.updateScheduleItem=()=>{let e=document.getElementById(`m_type`).value,i=document.getElementById(`itemSelectWrap`),a=document.getElementById(`m_item`),c=document.getElementById(`m_unit`),l={meat:{options:[[`kg`,`kg`],[`g`,`g`],[`마리`,`마리`]],default:`kg`},bag:{options:[[`박스`,`박스`],[`장`,`장`]],default:`박스`},egg:{options:[[`개`,`개`],[`판`,`판`]],default:`개`}};if(e&&l[e]){let t=l[e];c.innerHTML=t.options.map(([e,t])=>`<option value="${e}">${t}</option>`).join(``),c.value=t.default}else c.innerHTML=`<option value="">구분을 먼저 선택</option>`;if(e===`egg`){i.style.display=`none`,s();return}i.style.display=``,a.innerHTML=`<option value="">선택</option>`,(e===`meat`?r:o).filter(e=>e.active!==!1||n&&t?.itemId===e.id).forEach(e=>{a.innerHTML+=`<option value="${e.id}" data-name="${e.name}">${e.name}</option>`}),s()};let s=()=>{let e=document.getElementById(`unitGramsWrap`),t=document.getElementById(`m_unit_grams`),n=document.getElementById(`m_type`)?.value===`meat`&&document.getElementById(`m_unit`)?.value===`마리`;e&&(e.style.display=n?``:`none`),!n&&t&&(t.value=``)};document.getElementById(`m_unit`)?.addEventListener(`change`,s),n&&(document.getElementById(`m_type`).value=t.type||``,window.updateScheduleItem(),t.type!==`egg`&&(document.getElementById(`m_item`).value=t.itemId||``),document.getElementById(`m_unit`).value=t.orderedUnit||``,document.getElementById(`m_unit_grams`).value=t.orderedUnitGrams||``,s(),document.getElementById(`m_staff`).value=t.orderStaffName||``),document.getElementById(`btnSaveSchedule`).addEventListener(`click`,()=>b(async r=>{let o=g(r,a),{addDoc:s,updateDoc:l,recordActivity:u,recordMeatLog:d}=o,{getDoc:p}=r;if(!E()){alert(`입고 예정 ${n?`수정`:`등록`} 권한이 없습니다.`);return}let m=document.getElementById(`m_date`).value,h=n?t.type:document.getElementById(`m_type`).value,_=parseFloat(document.getElementById(`m_qty`).value),v=document.getElementById(`m_unit`).value,y=k(v)?parseFloat(document.getElementById(`m_unit_grams`)?.value):null,b=document.getElementById(`m_staff`).value,C=document.getElementById(`m_memo`).value;if(!m||!h||!_){alert(`날짜, 구분, 수량은 필수입니다.`);return}if(!b){alert(`담당자는 필수입니다.`);return}if(k(v)&&(!y||y<=0)){alert(`1마리당 g을 입력해주세요.`);return}if(n&&(await f(t.date,r)||m!==t.date&&await f(m,r)))return;let w=n&&t.itemId||null,T=n&&t.itemNameSnapshot||`계란`;if(!n&&h!==`egg`){let e=document.getElementById(`m_item`);if(w=e.value,T=e.options[e.selectedIndex]?.dataset?.name||``,!w){alert(`품목을 선택해주세요.`);return}}let D=(await S(r)).find(e=>n&&e.id===t.id||e.status!==`scheduled`||e.date!==m||e.type!==h?!1:h===`egg`?!0:e.itemId===w);if(D){alert(`이미 ${m}에 [${h===`egg`?`계란`:T}] 입고예정이 등록되어 있습니다.\n(기존: ${D.orderedQty}${D.orderedUnit})\n\n같은 날짜의 같은 품목은 중복 등록할 수 없습니다.`);return}if(n){let n={date:t.date,orderedQty:t.orderedQty,orderedUnit:t.orderedUnit,orderStaffName:t.orderStaffName||``,orderMemo:t.orderMemo||``};if(await l(e(a,`schedules`,t.id),{date:m,orderedQty:_,orderedUnit:v,orderedUnitGrams:y,orderStaffName:b,orderMemo:C,updatedAt:new Date}),await u({action:`schedule`,subAction:`edit`,date:c(),staff:b,message:`입고예정 수정 — ${T} ${n.orderedQty}${n.orderedUnit} → ${_}${v} (예정일 ${n.date} → ${m}) / 담당: ${b}`,details:{scheduleId:t.id,type:h,itemId:w,itemName:T,before:n,after:{date:m,orderedQty:_,orderedUnit:v,orderedUnitGrams:y,orderStaffName:b,orderMemo:C||``}}}),await o.commit(),!r.isCurrent())return;closeModal(),await x({force:!0}),alert(`입고 예정 수정 완료!`);return}let O=await s(i(a,`schedules`),{date:m,type:h,itemId:w,itemNameSnapshot:T,orderedQty:_,orderedUnit:v,orderedUnitGrams:y,status:`scheduled`,orderStaffName:b,orderMemo:C,createdAt:new Date,updatedAt:new Date}),A=h===`egg`?`계란`:`${h===`meat`?`원육`:h===`bag`?`봉투`:``} ${T}`;await u({action:`schedule`,subAction:`register`,date:c(),staff:b,message:`입고예정 등록 — ${A} ${_}${v} (예정일 ${m}) / 담당: ${b}`,details:{scheduleId:O.id,type:h,itemId:w,itemName:T,orderedQty:_,orderedUnit:v,orderedUnitGrams:y,scheduledDate:m,memo:C||null}}),await o.commit(),r.isCurrent()&&(closeModal(),await x({force:!0}),alert(`입고 예정 등록 완료!`))}))}function P(t){z(`
  ...
  `),document.getElementById(`btnSaveCancelSchedule`).addEventListener(`click`,()=>b(async n=>{let r=g(n,a),{addDoc:i,updateDoc:o,recordActivity:s,recordMeatLog:l}=r,{getDoc:u}=n;if(await f(t.date,n))return;let d=document.getElementById(`m_reason`).value.trim(),p=document.getElementById(`m_staff`).value;if(!d){alert(`취소 사유는 필수입니다.`);return}if(!p){alert(`담당자는 필수입니다.`);return}await o(e(a,`schedules`,t.id),{status:`cancelled`,cancelReason:d,cancelStaffName:p,cancelledAt:new Date,updatedAt:new Date}),await s({action:`schedule`,subAction:`cancel`,date:c(),staff:p,message:`${t.itemNameSnapshot} 입고 예정 취소 / 사유: ${d} / 담당: ${p}`,details:{scheduleId:t.id,type:t.type,itemId:t.itemId||null,itemName:t.itemNameSnapshot,orderedQty:t.orderedQty,unit:t.orderedUnit,scheduledDate:t.date,cancelReason:d}}),await r.commit(),n.isCurrent()&&(closeModal(),await x({force:!0}),alert(`취소 완료!`))}))}function F(t){let n=t.type===`meat`&&k(t.orderedUnit);z(`
  ...
  `),document.getElementById(`btnSaveComplete`).addEventListener(`click`,()=>b(async r=>{let o=g(r,a),{addDoc:s,updateDoc:l,recordActivity:d,recordMeatLog:p}=o,{getDoc:m}=r;try{let h=parseFloat(document.getElementById(`m_actual`).value),g=document.getElementById(`m_actual_unit`)?.value||t.orderedUnit,_=document.getElementById(`m_staff`).value,v=document.getElementById(`m_memo`).value;if(!h||!_){alert(`실제 수량과 담당자는 필수입니다.`);return}if(n&&(!t.orderedUnitGrams||t.orderedUnitGrams<=0)){alert(`이 발주의 1마리당 g 정보가 없습니다. 발주를 수정해 1마리당 g을 입력해주세요.`);return}let y=c();if(await f(y,r))return;let b=k(t.orderedUnit)&&g===`g`?Number(t.orderedQty||0)*Number(t.orderedUnitGrams||0):Number(t.orderedQty||0);if(h!==b&&!await u({title:`수량 차이 확인`,message:`발주 수량과 실제 수량이 다릅니다.\n\n발주: ${j(t.orderedQty,t.orderedUnit,t.orderedUnitGrams)}\n실제: ${j(h,g,t.orderedUnitGrams)}\n\n이대로 완료 처리하시겠습니까?`,confirmText:`완료 처리`}))return;let S=[];if(t.type===`meat`&&t.itemId){if((await m(e(a,`meatTypes`,t.itemId))).exists()){let e=k(t.orderedUnit)?g===`g`?h:h*Number(t.orderedUnitGrams||0):t.orderedUnit===`kg`?h*1e3:h,n=new Date,r=await s(i(a,`meatStocks`),{meatTypeId:t.itemId,meatNameSnapshot:t.itemNameSnapshot,stage:`frozen`,incomingDate:y,initialQtyG:e,remaining:e,staffName:_,note:`입고예정 완료: ${t.orderMemo||``}`,closed:!1,createdAt:new Date,updatedAt:n});await p({type:`frozenIncoming`,date:y,meatTypeId:t.itemId,meatNameSnapshot:t.itemNameSnapshot,stage:`frozen`,meatStockId:r.id,delta:e,before:0,after:e,staff:_,reason:`입고예정 완료${t.orderMemo?` - ${t.orderMemo}`:``}`}),S.push({collection:`meatStocks`,docId:r.id,field:`remaining`,delta:e,before:0,after:e,label:`${t.itemNameSnapshot} 냉동창고`,stockUpdatedAtSnapshot:n,isNewDoc:!0})}}else if(t.type===`bag`&&t.itemId){let n=await m(e(a,`bagTypes`,t.itemId));if(n.exists()){let r=n.data(),o=t.orderedUnit===`박스`?h*(r.piecesPerBox||1):h,c=r.currentQty||0,u=c+o,d=new Date;await l(e(a,`bagTypes`,t.itemId),{currentQty:u,updatedAt:d});let f=await s(i(a,`bagLogs`),{date:y,timestamp:new Date,bagTypeId:t.itemId,bagNameSnapshot:t.itemNameSnapshot,type:`incoming`,qty:o,before:c,after:u,staffName:_,note:`입고예정 완료`});S.push({collection:`bagTypes`,docId:t.itemId,field:`currentQty`,delta:o,before:c,after:u,label:`${t.itemNameSnapshot} 봉투`,stockUpdatedAtSnapshot:d,bagLogId:f.id})}}else if(t.type===`egg`){let n=await m(e(a,`eggStock`,`global`)),r=n.exists()?n.data().currentQty:0,o=n.exists()?n.data().minimumQty:0,c=t.orderedUnit===`판`?h*30:h,u=r+c,d=new Date;await l(e(a,`eggStock`,`global`),{currentQty:u,minimumQty:o,updatedAt:d});let f=await s(i(a,`eggLogs`),{date:y,timestamp:new Date,type:`in`,qty:c,before:r,after:u,staffName:_,note:`입고예정 완료`});S.push({collection:`eggStock`,docId:`global`,field:`currentQty`,delta:c,before:r,after:u,label:`계란`,stockUpdatedAtSnapshot:d,eggLogId:f.id})}let C=null;S.length>0&&(C=(await s(i(a,`stockLedger`),{actionType:`scheduleComplete`,actionId:t.id,timestamp:new Date,date:y,status:`active`,items:S})).id),await l(e(a,`schedules`,t.id),{status:`completed`,actualQty:h,actualUnit:g,actualQtyG:t.type===`meat`?k(t.orderedUnit)?g===`g`?h:h*Number(t.orderedUnitGrams||0):t.orderedUnit===`kg`?h*1e3:h:null,incomingStaffName:_,completeMemo:v,completedAt:new Date,ledgerId:C,updatedAt:new Date});let w=h!==b;if(await d({action:`schedule`,subAction:w?`completeDiff`:`complete`,date:y,staff:_,message:w?`${t.itemNameSnapshot} 입고 완료 ⚠️ 발주 ${j(t.orderedQty,t.orderedUnit,t.orderedUnitGrams)} → 실제 ${j(h,g,t.orderedUnitGrams)} / 담당: ${_}`:`${t.itemNameSnapshot} 입고 완료 차이 없음 / 담당: ${_}`,details:{scheduleId:t.id,type:t.type,itemId:t.itemId||null,itemName:t.itemNameSnapshot,orderedQty:t.orderedQty,actualQty:h,unit:g,orderedUnit:t.orderedUnit,orderedUnitGrams:t.orderedUnitGrams||null,hasDiff:w,memo:v||null}}),await o.commit(),!r.isCurrent())return;closeModal(),await x({force:!0}),alert(`완료 처리되었습니다!`)}catch(e){console.error(`입고 완료 처리 오류:`,e),alert(`완료 처리 중 오류가 발생했습니다.\n\n${e.code||``} ${e.message}\n\n이 메시지를 관리자에게 전달해주세요.`)}},[`admin`,`office`,`production`]))}function I(e){return String(e??``).replace(/&/g,`&amp;`).replace(/"/g,`&quot;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`)}var L={};function R(e){let t=``;for(let n of e)(L[n]||[]).forEach(e=>{t+=`<option value="${e.name}">${e.name}</option>`});return t}function z(e){let t=document.getElementById(`modalOverlay`);t&&t.remove();let n=document.createElement(`div`);n.id=`modalOverlay`,n.className=`modal-overlay`,n.innerHTML=`<div class="modal-box">${e}</div>`,document.body.appendChild(n),n.addEventListener(`click`,e=>{})}o(`schedule`,function(){let e=document.getElementById(`modalOverlay`);e&&e.remove()});export{x as renderSchedule};