import{b as e,d as t,s as n,u as r,y as i}from"./index.esm-rHmxwfvm.js";import{n as a}from"./firebase-qGjqjNvO.js";import{a as o}from"./formDraft-DoA-Ia7D.js";import{C as s,G as c,L as l,S as u,f as d,m as f}from"./index-lv3jHCmD.js";import{t as p}from"./closingGuard-CjIlTHvB.js";import{t as m}from"./readCommand-CFV9Wp6c.js";import{t as h}from"./pageRefresh-Dkf20J4p.js";import{t as g}from"./commandBatch-BvwieUXT.js";var _=f(`schedule`),v=[],y=[];async function b(e,t=[`admin`,`office`]){let n=u();try{return await m(_,t=>(t.isCurrent=()=>!n||n.isCurrent(),e(t)),{roles:t})}catch(e){console.error(`[입고 예정 저장]`,e),alert(e.message)}}async function x({force:e=!1}={}){let t=document.getElementById(`mainContent`);t.innerHTML=`<div style="padding:24px;"><p>입고 예정관리 로딩 중...</p></div>`;let n=await _.load(e=>B(e),{force:e,onChange:h(_,x)});!n||!t.isConnected||(v=n.meatTypes,y=n.bagTypes,L=n.staff,T(n.schedules))}async function S(e={getDocs:n}){let o=t(i(a,`schedules`),r(`date`,`asc`)),s=(await e.getDocs(o)).docs.map(e=>({id:e.id,...e.data()}));return s.sort((e,t)=>{if(e.date!==t.date)return e.date.localeCompare(t.date);let n=e.createdAt?.toMillis?e.createdAt.toMillis():0;return(t.createdAt?.toMillis?t.createdAt.toMillis():0)-n}),s}async function C(e={getDocs:n}){return(await e.getDocs(i(a,`meatTypes`))).docs.map(e=>({id:e.id,...e.data()}))}async function w(e={getDocs:n}){return(await e.getDocs(i(a,`bagTypes`))).docs.map(e=>({id:e.id,...e.data()}))}function T(e){let t=document.getElementById(`mainContent`),n=u(),r=l(),i=E(),a=e.filter(e=>e.status===`scheduled`),o=e.filter(e=>e.status!==`scheduled`),s=!1;t.innerHTML=`
    <div class="page-wrap">
      <div class="page-header">
        <h2 class="page-title">입고 예정관리</h2>
        ${i?`<button class="btn-primary" id="btnAddSchedule">+ 입고 예정 등록</button>`:``}
      </div>
      ...
      <div class="table-wrap" style="background:white;border-radius:8px;border:1px solid #e8e8e8;overflow:hidden;margin-bottom:16px;">
        <table class="data-table">
          ...
          <tbody>
            ${a.length===0?`<tr><td colspan="8" style="text-align:center;color:#aaa;padding:20px;">입고 예정 없음</td></tr>`:a.map(e=>D(e,r,i)).join(``)}
          </tbody>
        </table>
      </div>
      ...
      <div id="doneSection" style="display:none;">
        <div class="table-wrap" style="background:white;border-radius:8px;border:1px solid #e8e8e8;overflow:hidden;">
          <table class="data-table">
            ...
            <tbody>
              ${o.length===0?`<tr><td colspan="8" style="text-align:center;color:#aaa;padding:20px;">없음</td></tr>`:o.map(e=>O(e)).join(``)}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  `,document.getElementById(`btnAddSchedule`)?.addEventListener(`click`,()=>N()),document.getElementById(`btnToggleDone`).addEventListener(`click`,()=>{let e=document.getElementById(`doneSection`);s=!s,e.style.display=s?``:`none`,document.getElementById(`btnToggleDone`).textContent=s?`완료/취소 항목 숨기기`:`완료/취소 항목 보기`}),document.querySelectorAll(`.btn-complete`).forEach(t=>{t.addEventListener(`click`,async()=>{try{let n=t.dataset.id,r=e.find(e=>e.id===n);if(!r){alert(`일정을 찾을 수 없습니다. 새로고침 후 다시 시도해주세요.`);return}F(r)}catch(e){console.error(`완료 버튼 오류:`,e),alert(`오류가 발생했습니다: `+e.message)}})}),document.querySelectorAll(`.btn-edit-schedule`).forEach(t=>{t.addEventListener(`click`,async()=>{if(!E()){alert(`입고 예정 수정 권한이 없습니다.`);return}let r=t.dataset.id,i=e.find(e=>e.id===r);if(!i){alert(`입고 예정을 찾을 수 없습니다.`);return}await p(i.date)||n&&!n.isCurrent()||N(i)})}),document.querySelectorAll(`.btn-cancel-schedule`).forEach(t=>{t.addEventListener(`click`,async()=>{if(!E()){alert(`입고 예정 취소 권한이 없습니다.`);return}let r=t.dataset.id,i=e.find(e=>e.id===r);if(!i){alert(`입고 예정을 찾을 수 없습니다.`);return}await p(i.date)||n&&!n.isCurrent()||P(i)})})}function E(){return c===`admin`||c===`office`||c===`production`}function D(e,t,n=!1){let r=e.date<t;return`
