...
import { getEarliestUnclosedWorkday } from '../closing.js';
...
async function loadAllData() {
  ...
  overdueClosingDate = earliestUnclosed && earliestUnclosed < today ? earliestUnclosed : null;
  ...
  blockingData = await getAllBlockingItems(overdueClosingDate || today);
...
function renderMainLayout() {
  ...
  const activeDateLabel = isViewingSelectedDate
    ...
    : (isOverdueClosingMode ? `${overdueClosingDate} 생산 (미마감 처리 필요)`
    ...
    <div class="main-layout">
      <div class="main-panel-left">
        <div class="main-panel-header">
          <span class="main-panel-title">📅 ${todayStr} 생산${isViewingSelectedDate ? ' <span style="font-size:11px;color:#3182ce;font-weight:normal;">(선택 날짜)</span>' : ''}${isOverdueClosingMode ? ' <span style="font-size:11px;color:#c53030;font-weight:normal;">(미마감 처리 필요)</span>' : ''}</span>
          <div style="display:flex;gap:6px;align-items:center;">
            ...
            ${isViewingSelectedDate
              ...
              : (isOverdueClosingMode
                ? `<button class="btn-primary" id="btnTomorrowLoad" style="font-size:12px;padding:5px 14px;opacity:0.5;cursor:not-allowed;" disabled title="${overdueClosingDate} 마감 후 진행 가능">미마감 마감 필요</button>`
                : (isCompleted
                ? `
        ...
        <div class="main-production-area">
          ...
          <div class="main-production-grid">
            ${activeProductions.length === 0
              ? `<div class="main-empty">${isViewingSelectedDate ? '선택한 날짜에 생산 없음' : (isOverdueClosingMode ? '미마감 날짜에 생산 없음' : (isCompleted ? '불러온 다음 영업일 생산 없음' : '오늘 생산 없음'))}</div>`
...
function showDateModal(date) {
  ...
  document.getElementById('btnViewThisDate')?.addEventListener('click', () => {
    if (overdueClosingDate && date > overdueClosingDate) {
      alert(`${overdueClosingDate} 마감이 아직 처리되지 않았습니다.\n먼저 미마감 날짜를 처리한 뒤 다음 날짜를 확인하세요.`);
      selectedProductionDate = null;
      selectedDateProductions = [];
      closeModal();
      renderMainLayout();
